...
#define DEV_ESUMREPORT_DATATRANS_PERIOD			 		(30 * 1000) //电量上报周期，单位：ms
...
#define DEV_ENVIRONMENT_PARAM_REQUEST_PERIOD			(20 * 60) //环境参数获取请求周期，单位：s
...
#define DEVAPPSUPPLEMENT_A_FLG_BITHOLD_SOLARSYS_CTRL	(1 << 0) //太阳能电池管理器批量设备管控事件
#define DEVAPPSUPPLEMENT_A_FLG_DEVATMOS_LIGHT_CFG		(1 << 1) //边框氛围灯配置事件
#define DEVAPPSUPPLEMENT_A_FLG_DETECT_ALARM_REPORT		(1 << 2) //设备报警事件
#define DEVAPPSUPPLEMENT_A_FLG_CLUSTER_STATE_UPDATE		(1 << 3) //设备状态批量上报
#define DEVAPPSUPPLEMENT_A_FLG_AUTOMATION_SCENE_EXT		(1 << 4) //自动化场景执行
#define DEVAPPSUPPLEMENT_A_FLG_ENVIRO_PARAM_REQUEST		(1 << 5) //MQTT环境参数请求
#define DEVAPPSUPPLEMENT_A_FLG_CFGPARAM_SYNCHRO			(1 << 6) //设备设置参数主动同步
#define DEVAPPSUPPLEMENT_A_FLG_DEVICES_REBOOT					(1 << 6) //系统重启

#define DEVBUSSINESS_A_FLG_BITHOLD_RESERVE		 		(0xFFFF)

#define DEVBUSSINESS_A_FLG_BITHOLD_PSECEVT_HANDLE		(1 << 0) //软件定时器秒周期业务处理事件
#define DEVBUSSINESS_A_FLG_BITHOLD_NWK_RECONNECT_HAPPEN	(1 << 1) //网络重连发生事件

#define ICON_TIPSLOOPTIMER_APPEAR_TIME			6	//闹钟响应提示图标显示时间			，单位：s	
#define DEVDP_RESPOND_ACTION_DELAY_TIME			1	//设备数据点触发 --避免高频响应导致crash

#define TIME_LEGAL_MAXIMUM_HOUR 				24	//时间 小时 最大值
#define TIME_LEGAL_MAXIMUM_MINUTE 				60  //时间 分钟 最大值
 ...
 **********************/
 ...
 typedef struct{
	...
	uint8_t tScale_hour;
	uint8_t tScale_minute;
 }stt_timeScale;

 typedef struct{

	stt_timeScale sScale_sunRise;
	stt_timeScale sScale_sunSet;
 }stt_timeSunScale;
