...
static const char cmdTopic_m2s_list[DEVMQTT_TOPIC_NUM_M2S][DEVMQTT_TOPIC_CASE_LENGTH] = {
	...
	"/cmdDevListSet/opSolarManager/mulitStage",
	"/cmdDevListReq/opSolarManager/mulitStage",
	"/cmdCtrlParamSet/opSolarManager/mulitStage",
	"/cmdCtrlParamReq/opSolarManager/mulitStage",

	"/cmdAutomation/opSet",
	"/cmdAutomation/opReq",

	"/dispParamSet/opRgbLampBelt",
	"/dispParamReq/opRgbLampBelt",
	...
	"/cmdEpidemicNotice",
	"/cmdSunScaleTimeNotice",

	"/cmdSunScaleLocation/opSet",
	"/cmdSunScaleLocation/opReq",
};
enum{
	...
	cmdTopicM2SInsert_cmdDevListSet_opSSMRmulitStg,
	cmdTopicM2SInsert_cmdDevListReq_opSSMRmulitStg,
	cmdTopicM2SInsert_cmdCtrlParamSet_opSSMRstg,
	cmdTopicM2SInsert_cmdCtrlParamReq_opSSMRstg,

	cmdTopicM2SInsert_cmdAutomation_opSet,
	cmdTopicM2SInsert_cmdAutomation_opReq,

	cmdTopicM2SInsert_cmdDispParamSet_rgbLamp,
	cmdTopicM2SInsert_cmdDispParamReq_rgbLamp,
	...
	cmdTopicM2SInsert_cmdSunScaleTimeNotice,

	cmdTopicM2SInsert_cmdLocationSet,
	cmdTopicM2SInsert_cmdLocationReq,
};
...
static const char cmdTopic_s2m_list[DEVMQTT_TOPIC_NUM_S2M][DEVMQTT_TOPIC_CASE_LENGTH] = {
	...
	"/s2m/cmdDevListGet/opSolarManager/mulitStage",
	"/s2m/cmdCtrlParamGet/opSolarManager/mulitStage",

	"/s2m/cmdAutomation/opGet",

	"/s2m/dispParamGet/opRgbLampBelt",
	...
	"/cmdEpidemicCheck",
	"/cmdSunScaleTimeCheck",

	"/s2m/rootOpreatRespond",
};
enum{
	...
	cmdTopicS2MInsert_cmdDevListGet_opSSMRmulitStg,
	cmdTopicS2MInsert_cmdCtrlParamGet_opSSMRmulitStg,

	cmdTopicS2MInsert_cmdAutomation_opGet,

	cmdTopicM2SInsert_cmdDispParamGet_rgbLamp,

	cmdTopicS2MInsert_cmdDetectorAlarm,

	cmdTopicS2MInsert_cmdMeshUpgradeCheck,

	cmdTopicS2MInsert_cmdEpidemicCheck,
	cmdTopicS2MInsert_cmdSunScaleTimeCheck,
...
static bool remoteMqtt_connectFlg = false;
 ...
 void mqttRemoteCommunication_devSSMRcfgParamReport(void){
	 ...
	 dptr = communicationAppDataPkgGetWithBytes_devSSMRctrlParam(&dptrLen, &ctrlRunParam, false);
	 if(NULL != dptr){
	 	 memset(devMqtt_topicTemp, 0, sizeof(devMqtt_topicTemp));
		 sprintf(devMqtt_topicTemp, 
				 DEVMQTT_TOPIC_HEAD_A"MAC:%02X%02X%02X%02X%02X%02X%s", MAC2STR(devRouterBssid),
				 (char *)cmdTopic_s2m_list[cmdTopicS2MInsert_cmdCtrlParamGet_opSSMRmulitStg]);	 
		 printf("mqtt SSMR stg ctrlParam respRes:0x%04X.\n", esp_mqtt_client_publish(usrAppClient, devMqtt_topicTemp, (const char*)dptr, dptrLen, 1, 0));
		 os_free(dptr);
	 }
 }
#endif
...
//																			 dataTemp[5]);
		...
		case cmdTopicM2SInsert_cmdScenario_opCtrl:{
			...
			for(loop = 0; loop < scenarioUnit_num; loop ++){
				...
				if(!memcmp(devSelfMac, scenarioActionParam_unit.devMacAddr, MWIFI_ADDR_LEN)){
					...
					infraredOpreatActRemoteControlCmdLineUp(scenarioActionParam_unit.opreatVal & ~(1 << 7));
#else
					switch(currentDev_typeGet()){
					...
					currentDev_dataPointSet(&dataPointScenCtrl_temp, true, false, false, false, true); //场景操作不触发互控
#endif
				}
				else
				{
			...
			mqtt_rootDevRemoteDatatransStateListInfoRespond_trig(5); //场景控制后触发集群状态数据同步
		...
		case cmdTopicM2SInsert_cmdAutomation_opSet:{

			const uint8_t boardcastAddr[MWIFI_ADDR_LEN] = MWIFI_ADDR_BROADCAST;
			const uint16_t dataComming_lengthLimit = 23; //最小长度
			const uint16_t dataIndex_sceneParamStart = 23;
			uint8_t automationSetRes = 0;
			...
			devAutoExcuteParam_nvsTemp->dats->serialNum = event->data[0]; //自动化编号
			devAutoExcuteParam_nvsTemp->dats->existMask = event->data[1]; //编辑动作
			devAutoExcuteParam_nvsTemp->dats->automationTrigType = event->data[2]; //触发类型
			memcpy(&devAutoExcuteParam_nvsTemp->dats->effTime, (stt_aramAutomationEffectTime *)&event->data[3], sizeof(stt_aramAutomationEffectTime)); //有效时间
			memcpy(devAutoExcuteParam_nvsTemp->dats->conditionDevMac, (uint8_t *)&event->data[8], sizeof(uint8_t) * MWIFI_ADDR_LEN); //条件判定目标设备MAC
			memcpy(devAutoExcuteParam_nvsTemp->dats->conditionValue, (uint8_t *)&event->data[14], sizeof(uint8_t) * DEVAUTOMATION_TRIG_CONDITION_VAL_LEN); //自动化触发条件值
			devAutoExcuteParam_nvsTemp->dats->sceneDevNum = event->data[22]; //触发场景设备数量
			if(devAutoExcuteParam_nvsTemp->dats->sceneDevNum > 0){ //触发场景参数
				memcpy(devAutoExcuteParam_nvsTemp->dats->sceneParam, (stt_scenarioUnitOpreatParam *)&event->data[dataIndex_sceneParamStart], sizeof(uint8_t) * (event->data_len - dataIndex_sceneParamStart));
			}
			automationSetRes = nvsDataOpreation_devAutomationParam_set(devAutoExcuteParam_nvsTemp); //存储执行	
			printf(">>>automation param set: num:%d, type:%d, effectTime:hbit[%02XH]-Ah[%d]-Am[%d]-Bh[%d]-Bm[%d], condition0-[%02X], excuteDevNum:%d.\n",
				   devAutoExcuteParam_nvsTemp->dats->serialNum,
				   devAutoExcuteParam_nvsTemp->dats->automationTrigType,
				   devAutoExcuteParam_nvsTemp->dats->effTime.timeWeekBitHold,
				   devAutoExcuteParam_nvsTemp->dats->effTime.timeA_h,
				   devAutoExcuteParam_nvsTemp->dats->effTime.timeA_m,
				   devAutoExcuteParam_nvsTemp->dats->effTime.timeB_h,
				   devAutoExcuteParam_nvsTemp->dats->effTime.timeB_m,
				   devAutoExcuteParam_nvsTemp->dats->conditionValue[0],
				   devAutoExcuteParam_nvsTemp->dats->sceneDevNum);
			...
			if(NULL != dataMeshReq_bufTemp){
				...
				dataMeshReq_bufTemp[0] = L8DEV_MESH_CMD_AUTOMATION_SET; //mesh命令
				memcpy(&dataMeshReq_bufTemp[1], event->data, event->data_len); //数据
				...
				os_free(dataMeshReq_bufTemp); //数据操作缓存释放
			}
			...
			dataRespond_temp[0] = DATATRAN_MQTT_ROOTOP_RESPCODE_AUTOMATION_SET_RES;
			dataRespond_temp[1] = automationSetRes;
			printf("mqtt automation set respRes:0x%04X.\n", esp_mqtt_client_publish(event->client, devMqtt_topicTemp, (const char*)dataRespond_temp, 2, 1, 0));

		}break;

		case cmdTopicM2SInsert_cmdAutomation_opReq:{

			const uint16_t dataComming_lengthLimit = 1; //最小长度

			if(event->data_len < dataComming_lengthLimit)break;

			stt_automationExcutionNvsOp *devAutoExcuteParam_nvsTemp = (stt_automationExcutionNvsOp *)os_zalloc(sizeof(stt_automationExcutionNvsOp));
			uint16_t dpRespLen = 1 + sizeof(stt_paramAutomationExcution); //编号长度 + 数据长度
			uint8_t *dptrRespond = (uint8_t *)os_zalloc(dpRespLen); //编号长度 + 数据长度
			devAutoExcuteParam_nvsTemp->dats = (stt_paramAutomationExcution *)os_zalloc(sizeof(stt_paramAutomationExcution));
			devAutoExcuteParam_nvsTemp->opNum = event->data[0]; //这句不重要
			devAutoExcuteParam_nvsTemp->dats->serialNum = event->data[0]; //按自动化序列号读取
			nvsDataOpreation_devAutomationParamGet_bySerialNum(devAutoExcuteParam_nvsTemp);

			dptrRespond[0] = devAutoExcuteParam_nvsTemp->opNum;
			memcpy(&dptrRespond[1], devAutoExcuteParam_nvsTemp->dats, sizeof(stt_paramAutomationExcution));
			os_free(devAutoExcuteParam_nvsTemp->dats);
			os_free(devAutoExcuteParam_nvsTemp);
			
			//请求结果回复
			sprintf(devMqtt_topicTemp, DEVMQTT_TOPIC_HEAD_A"MAC:%02X%02X%02X%02X%02X%02X%s", MAC2STR(devRouterBssid),
																							 (char *)cmdTopic_s2m_list[cmdTopicS2MInsert_cmdAutomation_opGet]);
			printf("devAutomation excute param(Len:%d) respRes:0x%04X.\n", dpRespLen, esp_mqtt_client_publish(event->client, devMqtt_topicTemp, (const char *)dptrRespond, dpRespLen, 1, 0));
			os_free(dptrRespond);
		...
		case cmdTopicM2SInsert_cmdDevListSet_opSSMRmulitStg:{
...
#if(L8_DEVICE_TYPE_PANEL_DEF == DEV_TYPES_PANEL_DEF_SOLAR_SYS_MANAGER)
				communicationAppDataPkgSetToStruct_devSSMRctrlDevList((uint8_t *)event->data);
#endif
			}
			else
			{
				...
				if(NULL != dataMeshReq_bufTemp){
					...
					dataMeshReq_bufTemp[0] = L8DEV_MESH_CMD_SSMR_DEVLIST_STG_SET; //mesh命令
					memcpy(&dataMeshReq_bufTemp[1], event->data, event->data_len); //数据
		...
		case cmdTopicM2SInsert_cmdDevListReq_opSSMRmulitStg:{
			...
			if(!memcmp(devSelfMac, &(event->data[targetMacIstStart]), DEVICE_MAC_ADDR_APPLICATION_LEN)){
...
#if(L8_DEVICE_TYPE_PANEL_DEF == DEV_TYPES_PANEL_DEF_SOLAR_SYS_MANAGER)
				uint8_t stgMaskTemp = event->data[6];
				...
				if(NULL != dataTemp_solarSysManagerDevList){
					...
					sprintf(devMqtt_topicTemp, DEVMQTT_TOPIC_HEAD_A"MAC:%02X%02X%02X%02X%02X%02X%s", MAC2STR(devRouterBssid),
																									 (char *)cmdTopic_s2m_list[cmdTopicS2MInsert_cmdDevListGet_opSSMRmulitStg]);
					dptr = communicationAppDataPkgGetWithBytes_devSSMRctrlDevList(&dpLen, 
																				  ...
																				  false);
					if(NULL != dptr){
						printf("mqtt SSMR devListStg prt_A respRes:0x%04X.\n", esp_mqtt_client_publish(event->client, devMqtt_topicTemp, (const char *)dptr, dpLen, 1, 0));
						os_free(dptr);
					}
			...
			{
				dataRespond_temp[0] = L8DEV_MESH_CMD_SSMR_DEVLIST_STG_REQ; //mesh命令，没数据
				memcpy(&dataRespond_temp[1], (uint8_t *)event->data, event->data_len);
				
				ret = mwifi_root_write((const uint8_t *)&(event->data[targetMacIstStart]), 1,
									 &data_type, dataRespond_temp, event->data_len + 1, true);  //数据内容填充 -头命令长度1
				USR_ERROR_CHECK(ret != MDF_OK, ret, "<%s> mqtt mwifi_root_write", mdf_err_to_name(ret));
			}

		}break;
		
		case cmdTopicM2SInsert_cmdCtrlParamSet_opSSMRstg:{

			const uint8_t targetMacIstStart = 0; //数据包接收目标MAC地址起始索引
			const uint16_t dataComming_lengthLimit = 18; //数据长度最短限制
			
			if(event->data_len < dataComming_lengthLimit)break;

			if(!memcmp(devSelfMac, &(event->data[targetMacIstStart]), DEVICE_MAC_ADDR_APPLICATION_LEN)){

//				functionDebugDptrToHex_printf("ssmr ctrlParam Set:", (uint8_t *)event->data, event->data_len);

#if(L8_DEVICE_TYPE_PANEL_DEF == DEV_TYPES_PANEL_DEF_SOLAR_SYS_MANAGER)
				communicationAppDataPkgSetToStruct_devSSMRctrlParam((uint8_t *)event->data);
#endif
			}
			else
			{
				dataRespond_temp[0] = L8DEV_MESH_CMD_SSMR_CTRLPARAM_STG_SET; //mesh命令，没数据
				memcpy(&dataRespond_temp[1], event->data, event->data_len); //数据
				ret = mwifi_root_write((const uint8_t *)&(event->data[targetMacIstStart]), 1,
									 	&data_type, dataRespond_temp, event->data_len + 1, true);  //数据内容填充 -头命令长度1 + 操作值数据长度1
				USR_ERROR_CHECK(ret != MDF_OK, ret, "<%s> mqtt mwifi_root_write", mdf_err_to_name(ret));
			}

		}break;
		
		case cmdTopicM2SInsert_cmdCtrlParamReq_opSSMRstg:{
...
//							DEVMQTT_TOPIC_HEAD_A"MAC:%02X%02X%02X%02X%02X%02X%s", MAC2STR(devRouterBssid),
//							(char *)cmdTopic_s2m_list[cmdTopicS2MInsert_cmdCtrlParamGet_opSSMRmulitStg]);	
//					printf("mqtt SSMR stg ctrlParam respRes:0x%04X.\n", esp_mqtt_client_publish(event->client, devMqtt_topicTemp, (const char*)dptr, dptrLen, 1, 0));
//					os_free(dptr);
//				}
#if(L8_DEVICE_TYPE_PANEL_DEF == DEV_TYPES_PANEL_DEF_SOLAR_SYS_MANAGER)
				mqttRemoteCommunication_devSSMRcfgParamReport();
#endif
			}
			else
			{
				dataRespond_temp[0] = L8DEV_MESH_CMD_SSMR_CTRLPARAM_STG_REQ; //mesh命令，没数据
		...
		case cmdTopicM2SInsert_cmdDispParamSet_rgbLamp:{
   ...
   (L8_DEVICE_TYPE_PANEL_DEF == DEV_TYPES_PANEL_DEF_RGBLAMP_BULB)
				...
				uint8_t dpOpType = event->data[6]; //数据类型
				uint8_t *dptrParam = (uint8_t *)&event->data[7]; //数据实体
			
				devRgbLamp_dispTrickParam_paramSet(dptrParam, dpOpType, true);
				functionDebugDptrToHex_printf("devRgbLamp trickParam rcv", (uint8_t *)&event->data[6], 16);
#endif
			}
			else
			{
				...
				if(NULL != dataMeshReq_bufTemp){
					...
					dataMeshReq_bufTemp[0] = L8DEV_MESH_CMD_RGBLAMP_OP_PARAM_SET; //mesh命令
					memcpy(&dataMeshReq_bufTemp[1], event->data, event->data_len); //数据
		...
		case cmdTopicM2SInsert_cmdDispParamReq_rgbLamp:{
   ...
   (L8_DEVICE_TYPE_PANEL_DEF == DEV_TYPES_PANEL_DEF_RGBLAMP_BULB)
				...
				uint16_t dpRespLen = DEVICE_MAC_ADDR_APPLICATION_LEN +\
				...
				if(NULL != dptrResp){
					...
					memcpy(&dptrResp[ptrIndex], devSelfMac, sizeof(uint8_t) * DEVICE_MAC_ADDR_APPLICATION_LEN);
					ptrIndex += DEVICE_MAC_ADDR_APPLICATION_LEN;

					if(NULL != dpDevRgbLampTrickParam){

						paramUnitLen = dpRespLen - DEVICE_MAC_ADDR_APPLICATION_LEN;
						memcpy(&dptrResp[ptrIndex], dpDevRgbLampTrickParam, sizeof(uint8_t) * paramUnitLen);

						//请求结果回复
						memset(dataRespond_temp, 0, DEVMQTT_DATA_RESPOND_LENGTH);
						sprintf(devMqtt_topicTemp, DEVMQTT_TOPIC_HEAD_A"MAC:%02X%02X%02X%02X%02X%02X%s", MAC2STR(devRouterBssid),
																										 (char *)cmdTopic_s2m_list[cmdTopicM2SInsert_cmdDispParamGet_rgbLamp]);
						printf("rgbLamp opParam(Len:%d) respRes:0x%04X.\n", dpRespLen, esp_mqtt_client_publish(event->client, devMqtt_topicTemp, (const char*)dptrResp, dpRespLen, 1, 0));
			...
			{
				dataRespond_temp[0] = L8DEV_MESH_CMD_RGBLAMP_OP_PARAM_REQ; //mesh命令，没数据
...
//			lvGui_usrAppBussinessRunning_block(2, "\nHA server changed", 6);	
		...
		case cmdTopicM2SInsert_cmdMeshUpgradeForce:{
			...
			if(checkNum == (uint8_t)event->data[numCheck_len]){
				...
				usrAppUpgrade_targetDevaddrSet((const uint8_t *)&event->data[targetMacIstStart]); //单独升级mac地址修改
				...
				devNode = L8devHbDataManageList_nodeGet(listHead_nodeDevDataManage, (uint8_t *)&event->data[targetMacIstStart], true); //链表节点获取
				if((NULL != devNode) || !memcmp(devSelfMac, (uint8_t *)&event->data[targetMacIstStart], MWIFI_ADDR_LEN)){ //设备存在才进行升级触发
					printf("devTyep[%02X] upgrade trig now!!!.\n", devNode->dataManage.nodeDev_Type);
					usrApp_firewareUpgrade_trig(false, devNode->dataManage.nodeDev_Type); //按照指定类型进行升级触发				
				}else{
					ESP_LOGW(TAG, "force upgrade fail, target devMac["MACSTR"] not found.\n", MAC2STR((uint8_t *)&event->data[targetMacIstStart]));
				}
			}
...
//																									  epidDataTemp.sumData_deaths);
		...
		case cmdTopicM2SInsert_cmdSunScaleTimeNotice:{

			const uint8_t boardcastAddr[MWIFI_ADDR_LEN] = MWIFI_ADDR_BROADCAST;
			const uint16_t dataComming_lengthLimit = 4;
			stt_timeSunScale sunScaleTemp = {0};
			
			if(event->data_len < dataComming_lengthLimit)break; //数据长度最短限制

			printf(">>>sun scale time info got[len:%d]:%02XH %02XH %02XH %02XH.\n", event->data_len,
																					event->data[0],
																					event->data[1],
																					event->data[2],
																					event->data[3]);

			memcpy(&sunScaleTemp, (stt_timeSunScale *)event->data, sizeof(stt_timeSunScale));
//			sunScaleTemp.sScale_sunRise.tScale_hour		= event->data[0];
//			sunScaleTemp.sScale_sunRise.tScale_minute	= event->data[1];
//			sunScaleTemp.sScale_sunSet.tScale_hour		= event->data[2];
//			sunScaleTemp.sScale_sunSet.tScale_minute	= event->data[3];
			deviceParamSet_sunScale(&sunScaleTemp, true);
			printf(">>>sun scale time info set:%02d %02d %02d %02d.\n", (int)sunScaleTemp.sScale_sunRise.tScale_hour,
																																				(int)sunScaleTemp.sScale_sunRise.tScale_minute,
																																				(int)sunScaleTemp.sScale_sunSet.tScale_hour,
																																				(int)sunScaleTemp.sScale_sunSet.tScale_minute);
			
			if(1 < esp_mesh_get_total_node_num()){
				dataRespond_temp[0] = L8DEV_MESH_CMD_ENVIR_INFO_BOARDCAST; //mesh命令
				memcpy(&dataRespond_temp[1], (uint8_t *)event->data, event->data_len); //数据
				ret = mwifi_root_write(boardcastAddr, 1, &data_type, dataRespond_temp, event->data_len + 1, true);
				USR_ERROR_CHECK(ret != MDF_OK, ret, "<%s> mqtt mwifi_root_write", mdf_err_to_name(ret)); 
			}
	
		}break;

		case cmdTopicM2SInsert_cmdLocationSet:{

			if(event->data_len < 1)break;
			else{

				const uint8_t boardcastAddr[MWIFI_ADDR_LEN] = MWIFI_ADDR_BROADCAST;

				char *infoStr = (char *)os_zalloc(sizeof(char) * SCALE_LOCATION_INFO_STR_MAX_LENGTH);
				if(event->data_len >= SCALE_LOCATION_INFO_STR_MAX_LENGTH)
					printf(">>>sun scale location info too long, len:%d.\n", event->data_len);
				memcpy(infoStr, event->data, (event->data_len >= SCALE_LOCATION_INFO_STR_MAX_LENGTH)?(sizeof(char) * SCALE_LOCATION_INFO_STR_MAX_LENGTH - 1):(sizeof(char) * event->data_len));
				devSunScaleLocationInfo_set(infoStr, true);	
				printf(">>>sun scale location info got[len:%d]:%s.\n", event->data_len, infoStr);
				free(infoStr);

				dataRespond_temp[0] = L8DEV_MESH_CMD_SUNSCALE_LOCATION_INFO_BOARDCAST; //mesh命令
				memcpy(&dataRespond_temp[1], (uint8_t *)event->data, event->data_len); //数据
				ret = mwifi_root_write(boardcastAddr, 1, &data_type, dataRespond_temp, event->data_len + 1, true);
				USR_ERROR_CHECK(ret != MDF_OK, ret, "<%s> mqtt mwifi_root_write", mdf_err_to_name(ret)); 

				xEventGroupSetBits(xEventGp_devAppSupplemet_A, DEVAPPSUPPLEMENT_A_FLG_ENVIRO_PARAM_REQUEST); //每次设置过位置信息后都重新获取日出日落时间
			}

		}break;
		
		case cmdTopicM2SInsert_cmdLocationReq:{

//			devSunScaleLocationInfo_get();
...
//			ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);
			...
			devEnvironmentParamReqByMqttExcuteTrig(3); //每次上线都进行一次环境参数获取
			
			homeassistantApp_devOnlineSynchronous(client, devSelfMac);

			xEventGroupSetBits(xEventGp_devBussinessHandle_A, DEVBUSSINESS_A_FLG_BITHOLD_NWK_RECONNECT_HAPPEN); //mqtt重连事件
...
void mqtt_rootDevSunScaleTimeParamRequestExcution(void){

	uint8_t devRouterBssid[MWIFI_ADDR_LEN] = {0};
	char *mqttDataReqTemp = (char *)os_zalloc(sizeof(char) * SCALE_LOCATION_INFO_STR_MAX_LENGTH);
	uint16_t mqttData_pbLen = 0;

	if(NULL == mqttDataReqTemp)return;
	if(mwifi_is_connected() && esp_mesh_get_layer() == MESH_ROOT);
	else{
	...
	memset(devMqtt_topicTemp, 0, DEVMQTT_TOPIC_TEMP_LENGTH); //主题缓存清零
	sprintf(devMqtt_topicTemp, DEVMQTT_TOPIC_HEAD_A"MAC:%02X%02X%02X%02X%02X%02X%s", MAC2STR(devRouterBssid),
												(char *)cmdTopic_s2m_list[cmdTopicS2MInsert_cmdSunScaleTimeCheck]);
	
	devSunScaleLocationInfo_get((char *)mqttDataReqTemp);
	mqttData_pbLen = strlen((char *)mqttDataReqTemp);

	printf("mqtt devSunscle time param check res:0x%04X.\n", esp_mqtt_client_publish(usrAppClient, devMqtt_topicTemp, (const char*)mqttDataReqTemp, mqttData_pbLen, 1, 0)); //数据发送

	free(mqttDataReqTemp);
}

void mqtt_rootDevRemoteDatatrans_devStateListInfoRespond(void){
	...
	if(devUnitNum_temp == DEVLIST_MANAGE_LISTNUM_MASK_NULL){ //设备采集未就绪，设备还未足够运行一个心跳周期
		...
		mqttData_pbLen = 1;
		printf("mqtt cmdQuery_devStatus respondNoReady res:0x%04X.\n", esp_mqtt_client_publish(usrAppClient, devMqtt_topicTemp, (const char*)dataResp_devStatusInfo, mqttData_pbLen, 1, 0));
	}
	else
	{
		uint8_t dataRespLoop = devUnitNum_temp / devUnitNumLimit_perPack; //商
...
void mqtt_rootDevRemoteDatatransStateListInfoRespond_trig(uint8_t countDown_s){

	usrAppDevClusterStateUpdataCountingDown_trig(countDown_s);
}
...
void mqtt_remoteDataTrans(uint8_t dtCmd, uint8_t *data, uint16_t dataLen){
	...
	enum{
		...
		cmdQuery_devListStgSSMR = L8DEV_MESH_CMD_SSMR_DEVLIST_STG_REQ,
		cmdQuery_devCtrlParamStgSSMR = L8DEV_MESH_CMD_SSMR_CTRLPARAM_STG_REQ,
		cmdQuery_devRgbLampParam = L8DEV_MESH_CMD_RGBLAMP_OP_PARAM_REQ,
		cmdReport_devDetectorAlarm = L8DEV_MESH_CMD_DETECTOR_ALARMREPORT,
	...
	if(remoteMqtt_connectFlg){ //远程连接是否可用？
		...
		switch(dtCmd){
			...
			case cmdQuery_devListStgSSMR:{ //缓存不够，直接使用源数据

				sprintf(devMqtt_topicTemp, DEVMQTT_TOPIC_HEAD_A"MAC:%02X%02X%02X%02X%02X%02X%s", MAC2STR(devRouterBssid),
																								 (char *)cmdTopic_s2m_list[cmdTopicS2MInsert_cmdDevListGet_opSSMRmulitStg]);
				printf("mqtt SSMR devListStg responed res:%d.\n", esp_mqtt_client_publish(usrAppClient, devMqtt_topicTemp, (const char*)data, dataLen, 1, 0));

			}break;

			case cmdQuery_devCtrlParamStgSSMR:{
				
				sprintf(devMqtt_topicTemp, DEVMQTT_TOPIC_HEAD_A"MAC:%02X%02X%02X%02X%02X%02X%s", MAC2STR(devRouterBssid),
																								 (char *)cmdTopic_s2m_list[cmdTopicS2MInsert_cmdCtrlParamGet_opSSMRmulitStg]);
				printf("mqtt SSMR ctrlParam responed res:%d.\n", esp_mqtt_client_publish(usrAppClient, devMqtt_topicTemp, (const char*)data, dataLen, 1, 0));

			}break;

			case cmdQuery_devRgbLampParam:{ //缓存不够，直接使用源数据

				sprintf(devMqtt_topicTemp, DEVMQTT_TOPIC_HEAD_A"MAC:%02X%02X%02X%02X%02X%02X%s", MAC2STR(devRouterBssid),
																								 (char *)cmdTopic_s2m_list[cmdTopicM2SInsert_cmdDispParamGet_rgbLamp]);
				printf("mqtt rgbLamp opParam responed res:%d.\n", esp_mqtt_client_publish(usrAppClient, devMqtt_topicTemp, (const char*)data, dataLen, 1, 0));

			}break;

			case cmdReport_devDetectorAlarm:{

				dtCmd_identify = true;
				sprintf(devMqtt_topicTemp, DEVMQTT_TOPIC_HEAD_A"MAC:%02X%02X%02X%02X%02X%02X%s", MAC2STR(devRouterBssid),
																								 (char *)cmdTopic_s2m_list[cmdTopicS2MInsert_cmdDetectorAlarm]);
			}break;

			case cmdReport_statusSynchro:{

				stt_nodeDev_hbDataManage *devNode = NULL;
				uint8_t devMacAddrDevNode[MWIFI_ADDR_LEN] = {0};
				uint8_t devTypeTemp = devTypeDef_null;

				memcpy(devMacAddrDevNode, &data[5], sizeof(uint8_t) * MWIFI_ADDR_LEN); //获取MAC地址，从index_5开始
				devNode = L8devHbDataManageList_nodeGet(listHead_nodeDevDataManage, devMacAddrDevNode, false);
				if(NULL != devNode){
					devTypeTemp = devNode->dataManage.nodeDev_Type;
					if(devTypeDef_scenario == devNode->dataManage.nodeDev_Type){ //若是场景开关状态改变，则进行一次集体状态上报
						printf("scenario sw sycn, trig devList state report.\n");
						mqtt_rootDevRemoteDatatransStateListInfoRespond_trig(4);
					}
					free(devNode);
				}

				//自动化触发检测-条件：设备状态
				devAutomationExcutionDetectJudge_byConditionDevState(devMacAddrDevNode, devTypeTemp, data[0]); 
...
void usrApp_devRootStatusSynchronousInitiative(void){
	...
	if(true == devDriverBussiness_solarSysManager_paramCfgSyncValid()){ //设置数据上传可用，被触发
		currentDev_extParamGet_x(&dataTrans_temp[1], 1);
		mqtt_remoteDataTrans(L8DEV_MESH_STATUS_SYNCHRO, dataTrans_temp, dataTrans_tempLen);
		vTaskDelay(20 / portTICK_RATE_MS);
	}
	currentDev_extParamGet_x(&dataTrans_temp[1], 0);
#else
...
void usrApp_devRootCfgParamSynchronousInitiative(void){
   ...
   (L8_DEVICE_TYPE_PANEL_DEF == DEV_TYPES_PANEL_DEF_PIR_DETECTOR)
 void usrApp_devRootDetectingAlarmReportInitiative(void){

	uint8_t *dptr = (uint8_t *)os_zalloc(sizeof(stt_devDetectingDataDef));

	if(NULL != dptr){

		currentDev_detectingReportDataGet((stt_devDetectingDataDef *)dptr);
		mqtt_remoteDataTrans(L8DEV_MESH_CMD_DETECTOR_ALARMREPORT, dptr, sizeof(stt_devDetectingDataDef));
		os_free(dptr);
	}
 }
#endif
...
void usrApp_deviceStatusSynchronousInitiative(void){
	...
	if(mwifi_is_connected()){
		...
		devDriverBussinessGasDetect_flgSycnUpdated_set(true);

#elif(L8_DEVICE_TYPE_PANEL_DEF == DEV_TYPES_PANEL_DEF_SMOKE_DETECTOR)

		devDriverBussinessSmokeDetect_flgSycnUpdated_set(true);

#elif(L8_DEVICE_TYPE_PANEL_DEF == DEV_TYPES_PANEL_DEF_PIR_DETECTOR)

		devDriverBussinessPirDetect_flgSycnUpdated_set(true);
#endif
	}
}

void usrApp_deviceCfgParamSynchronousInitiative(void){

	if(mwifi_is_connected()){

		if(esp_mesh_get_layer() == MESH_ROOT){

			usrApp_devRootCfgParamSynchronousInitiative();
		}
		else
		{
			usrApp_devNodeCfgParamSynchronousInitiative();
		}
	}
}

void usrApp_meshCommunicationRequestAllDevicesReboot(void){
	...
	if(false == mwifi_is_connected())return; //当前网络不可用
		...
		size_t	dataTX_len = 1 + 1;		//命令头 + 重启等待时间
		uint8_t *dataTX_temp = (uint8_t *)LV_MEM_CUSTOM_ALLOC(sizeof(uint8_t) * dataTX_len);

		memcpy(&data_type.custom, &type_L8mesh_cst, sizeof(uint32_t));

		dataTX_temp[0] = L8DEV_MESH_CMD_SYSTEM_REBOOT;
		dataTX_temp[1] = 5;
   ...
   (L8_DEVICE_TYPE_PANEL_DEF == DEV_TYPES_PANEL_DEF_SMOKE_DETECTOR)||\
   (L8_DEVICE_TYPE_PANEL_DEF == DEV_TYPES_PANEL_DEF_PIR_DETECTOR)
 void usrApp_deviceDetectingAlarmReportInitiative(void){

	if(mwifi_is_connected()){

		if(esp_mesh_get_layer() == MESH_ROOT){

			usrApp_devRootDetectingAlarmReportInitiative();
		}
		else
		{
			usrApp_devNodeDetectingAlarmReportInitiative();
		}
	}
 }
#endif
...
//	esp_log_level_set(TAG, ESP_LOG_INFO);
	...
	else
	{
		...
		// mqtt_cfg.port = 1883;
		// printf(">>>trying to connect broker'%s', port:%d.\n", mqtt_cfg.host, mqtt_cfg.port);
	}
