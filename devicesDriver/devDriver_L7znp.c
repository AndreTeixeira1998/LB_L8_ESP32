...
//											ESP_LOGI(TAG, ">>>devNode refresh cdcnt:%d.", ZigbDevNew_temp->devStateRefreshCdCnt);
									...
									//注意数据包类型，不同包属性数据位置不一样
									if((local_datsRX->datsSTT.stt_MSG.dats[0] == DTMODEKEEPACESS_FRAMEHEAD_ONLINE) ||
									   (local_datsRX->datsSTT.stt_MSG.dats[0] == DTMODEKEEPACESS_FRAMEHEAD_OFFLINE)){

										ZigbDevNew_temp->devDefClass = local_datsRX->datsSTT.stt_MSG.dats[9]; //设备记录类型更新
										//ESP_LOGI(TAG, "devType refresh:%02X", ZigbDevNew_temp->devDefClass);
									}
