...
void devDriverBussiness_mulitBitSwitch_moudleInit(void){
	...
	if((swCurrentDevType != devTypeDef_mulitSwOneBit)&&
	   ...
	   (swCurrentDevType != devTypeDef_largeCurOneBit)&& 
	   (swCurrentDevType != devTypeDef_moudleSwOneBit)&&
	   (swCurrentDevType != devTypeDef_moudleSwTwoBit)&&
	   (swCurrentDevType != devTypeDef_moudleSwThreeBit)&&
	   (swCurrentDevType != devTypeDef_relayBox_1bit)&&
	   (swCurrentDevType != devTypeDef_relayBox_2bit))return;
...
//#else
	...
	if(statusVal_opreat & 0xE0){ //是否为语音控制操作（高三位有效？）
		...
		currentDev_dataPointRecovery(&dataPoint_temp, false, true, false, false, false); //按位操作数据点恢复
	}
	else
	{
		switch(swCurrentDevType){
