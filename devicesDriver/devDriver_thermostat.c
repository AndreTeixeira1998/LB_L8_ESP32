...
static enumDevThermostat_realyAction deviceThermostatRelayActionState = devThermRealyAction_null;

static float temperatureValTargetFahrenheit = 0.0F;
...
void devDriverBussiness_thermostatSwitch_runningDetectLoop(void){
	...
	if((tempratureRead_temp > 64.0F) || (tempratureRead_temp < 0.0F)) //合理判断
		devParam_thermostat.temperatureVal_current = 0;
	else
		...
		if(enableRecord){
			...
			switch(currentDev_typeGet()){
				...
				case devTypeDef_thermostat:{
					...
					DEVDRIVER_THERMOSTAT_ADJUST_STOP();
					deviceThermostatRelayActionState = devThermRealyAction_close;
			...
			switch(currentDev_typeGet()){
				...
				case devTypeDef_thermostat:{
					...
					if(devParam_thermostat.temperatureVal_current <= (devParam_thermostat.temperatureVal_target - adjustTrig_shortVal)){
						...
						deviceThermostatRelayActionState = devThermRealyAction_heat;
					}
					else
			...
			switch(currentDev_typeGet()){
				...
				case devTypeDef_thermostat:{
					...
					if(devParam_thermostat.temperatureVal_current >= (devParam_thermostat.temperatureVal_target + adjustTrig_shortVal)){
						...
						deviceThermostatRelayActionState = devThermRealyAction_cool;
					}
					else
					if(devParam_thermostat.temperatureVal_current < devParam_thermostat.temperatureVal_target){
...
enumDevThermostat_realyAction deviceThermostatRelayActionState_get(void){
	return deviceThermostatRelayActionState;
}

float *devDriverBussiness_thermostatSwitch_paramRcdFahrenheitGet(void){
	return &temperatureValTargetFahrenheit;
}

void devDriverBussiness_thermostatSwitch_paramRcdFahrenheitRefresh(void){
	temperatureValTargetFahrenheit = varConverionCelsius2Fahrenheit_f((float)devParam_thermostat.temperatureVal_target);
}
