...
#define	DEV_L7_UDP_REMOTE_COMMUNICATION_PORT	3000
...
static xTaskHandle pxTaskHandle_l7SocketProcess = NULL;

struct udp_pcb *socketLocal_pcb = NULL,
			   *socketRemote_pcb = NULL;
ip_addr_t socketLocal_remoteIp = {0}, 
		  socketLocal_ipObjTemp = {0},
		  socketRemote_remoteIp = {0};
struct pbuf *socketLocal_payload = NULL,
			*socketRemote_payload = NULL;
...
static void udpRemoteOperateDataSend(uint8_t *dats, uint16_t datsLen){
	...
	socketRemote_payload = pbuf_alloc(PBUF_TRANSPORT, sizeof(uint8_t) * datsLen, PBUF_RAM);
	//memcpy(socketRemote_payload->payload, dats, sizeof(uint8_t) * datsLen);
	socketRemote_payload->payload = dats;
	udp_sendto(socketRemote_pcb, socketRemote_payload, &socketRemote_remoteIp, DEV_L7_UDP_REMOTE_COMMUNICATION_PORT);
	pbuf_free(socketRemote_payload);
	socketRemote_payload = NULL;
}

static void udpLocalOperateDataSend(uint8_t *dats, uint16_t datsLen){

	socketLocal_payload = pbuf_alloc(PBUF_TRANSPORT, sizeof(uint8_t) * datsLen, PBUF_RAM);
	//memcpy(socketLocal_payload->payload, dats, sizeof(uint8_t) * datsLen);
	socketLocal_payload->payload = dats;
	//udp_sendto(socketLocal_pcb, socketLocal_payload, &socketLocal_remoteIp, DEV_L7_UDP_LOCAL_COMMUNICATION_PORT);
	udp_sendto(socketLocal_pcb, socketLocal_payload, &socketLocal_ipObjTemp, DEV_L7_UDP_LOCAL_COMMUNICATION_PORT);
	pbuf_free(socketLocal_payload);
	socketLocal_payload = NULL;
}
...
static STATUS sockets_datsSend(socket_OBJ sObj, uint8_t dats[], uint16_t datsLen){
	...
	if(true == internet_connFLG){
		...
		ESP_LOGI(TAG, "socket send happen, type:%d", (int)sObj);
...
static void udpProcessRemoteDataRcvHandle(uint8_t *pdata, uint16_t len){
	...
	else
	{
		...
		//functionDebugDptrToHex_printf("[Tips_socketUDP_B]", pdata, len);
	}	
}
...
static void udpProcessLocalDataRcvHandle(uint8_t *pdata, uint16_t len){
	...
	//functionDebugDptrToHex_printf(TAG, pdata, len);	
...
static void udp_rcv_callback_dataTransLocal(		void *arg, 
	...
	memcpy(&socketLocal_ipObjTemp, &my_ipaddr, sizeof(ip_addr_t));

	//ESP_LOGI(TAG, "udp local data rcv, %s", (char *)p->payload);
	//ESP_LOGI(TAG, "udp local data rcv, len:%d", (int)p->len);
	//udp_sendto(pcb, p, &my_ipaddr, port); // 将报文返回给原主机

	udpProcessLocalDataRcvHandle((uint8_t *)p->payload, p->len);
...
static void devL7SmartlivingUdpLocalCommunicationProcessTask(void *pvParameters){
	...
	internet_connFLG = true;

	p = pbuf_alloc(PBUF_TRANSPORT, strlen(dptr), PBUF_RAM);
	tcpip_adapter_get_ip_info(TCPIP_ADAPTER_IF_STA, &ip_info); //ip_info.ip.addr;
	ESP_LOGI(TAG, "connected ip got:"IPSTR, IP2STR(&ip_info.ip));

	socketLocal_pcb = udp_new();
	if(socketLocal_pcb == NULL){
		ESP_LOGE(TAG, "udp local creat fail.");
	}else{
		//memcpy(&socketLocal_remoteIp, &ip_info.ip.addr, sizeof(u32_t));
		//IP4_ADDR(ip_2_ip4(&socketLocal_remoteIp), 192, 168, 0, 121);
		//udp_disconnect(socketLocal_pcb);
		socketLocal_remoteIp = *IP_ADDR_ANY;
		
		if(ERR_OK == udp_connect(socketLocal_pcb, &socketLocal_remoteIp, DEV_L7_UDP_LOCAL_COMMUNICATION_PORT)){ // 连接到指定的IP地址和端口

			//socketLocal_pcb->local_port = 8888; //自动端口
			socketLocal_pcb->local_port = DEV_L7_UDP_LOCAL_COMMUNICATION_PORT;
			udp_recv(socketLocal_pcb, udp_rcv_callback_dataTransLocal, NULL); // 注册报文处理回调				
			ESP_LOGI(TAG, "udp local connected, local_port %d.", (int)socketLocal_pcb->local_port);
		}else{
	...
	for(;;){
		...
		udp_sendto(socketLocal_pcb, p, &socketLocal_remoteIp, DEV_L7_UDP_LOCAL_COMMUNICATION_PORT);
		vTaskDelay(2000 / portTICK_PERIOD_MS);
	}
...
static void udp_rcv_callback_dataTransRemote(		void *arg, 
	...
	//ESP_LOGI(TAG, "udp remote data rcv, Len:%d", (int)p->len);
	//udp_sendto(pcb, p, &my_ipaddr, port); // 将报文返回给原主机

	udpProcessRemoteDataRcvHandle((uint8_t *)p->payload, p->len);
...
static void devL7SmartlivingUdpRemoteCommunicationProcessTask(void *pvParameters){
	...
	socketRemote_pcb = udp_new();
	if(socketRemote_pcb == NULL){
		ESP_LOGE(TAG, "udp remote creat fail.");
	}else{
		IP4_ADDR(ip_2_ip4(&socketRemote_remoteIp), 47, 52, 5, 108);
//		IP4_ADDR(ip_2_ip4(&socketRemote_remoteIp), 112, 124, 61, 191);
		//udp_disconnect(socketRemote_pcb);
		if(ERR_OK == udp_connect(socketRemote_pcb, &socketRemote_remoteIp, DEV_L7_UDP_REMOTE_COMMUNICATION_PORT)){ // 连接到指定的IP地址和端口

//			socketRemote_pcb->local_port = 12345; //自动端口
			udp_recv(socketRemote_pcb, udp_rcv_callback_dataTransRemote, NULL); // 注册报文处理回调				
			ESP_LOGI(TAG, "udp remote connected, local_port %d.", socketRemote_pcb->local_port);
		}else{
	...
	for(;;){
		...
		udp_sendto(socketRemote_pcb, p, &socketRemote_remoteIp, DEV_L7_UDP_REMOTE_COMMUNICATION_PORT);
		vTaskDelay(2000 / portTICK_PERIOD_MS);
	}
				   ...
				   (rptr_socketDats.dstObj == obj_toALL)){ 
					...
					switch(rptr_socketDats.command){
						...
						case FRAME_MtoSCMD_cmdConfigSearch:{
							...
							socketRespond_IF	= false; //只做主机，不做设备
...
void devL7SmartlivingUdp_remoteComunication_initialization(void){
	...
	xRes = xTaskCreate(devL7SmartlivingSocketsDataTransProcessTask, "udpTaskSktProcess", 1024 * 6, (void *)NULL, 5, &pxTaskHandle_l7SocketProcess);
