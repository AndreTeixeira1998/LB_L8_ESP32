...
//	}
	...
	if(!tempMeasure_startFlg){ //起始采样
		...
		if(tempSample_new < 60.0F){ //首次采样为样本值，必须相对标准
...
float devTempratureSensor_dataCal_hanldeGet(uint8_t param){
	...
	(devTempDispMethod_unit_get())? //中间范围
		(ret = (float)param / 10.0f - (tempCal_maxRange / 2.0f)):
		(ret = (float)param / 10.0f - (tempCal_maxRange / 2.0f));
...
uint16_t varConverionFahrenheit2Celsius(uint16_t var){

	float varTmpFahrenheit = 0.0F;

	varTmpFahrenheit = ((float)var - 32.0F) / 1.8F;

	return (uint16_t)varTmpFahrenheit;
}

uint16_t varConverionCelsius2Fahrenheit(uint16_t var){

	float varTmpCelsius = 0.0F;

	varTmpCelsius = (float)var * 1.8F + 32.0F;

	return (uint16_t)varTmpCelsius;
}

float varConverionFahrenheit2Celsius_f(float var){

	float varTmpFahrenheit = 0.0F;

	varTmpFahrenheit = (var - 32.0F) / 1.8F;

	return varTmpFahrenheit;
}

float varConverionCelsius2Fahrenheit_f(float var){

	float varTmpCelsius = 0.0F;

	varTmpCelsius = (var * 1.8F) + 32.0F;

	return varTmpCelsius;
}
