...
static stt_devFan_paramCfg devFanCfgParam = {30, 60, 100};
...
void devDriverBussiness_fanSwitch_cfgParamGet(stt_devFan_paramCfg *param){
	...
	memcpy(param, &devFanCfgParam, sizeof(stt_devFan_paramCfg));
}

void devDriverBussiness_fanSwitch_cfgParamSet(stt_devFan_paramCfg *param, bool nvsRecord_IF){

	memcpy(&devFanCfgParam, param, sizeof(stt_devFan_paramCfg));

	if(nvsRecord_IF){
		devSystemInfoLocalRecord_save(saveObj_devFan_cfgParam, &devFanCfgParam);
	}
}
	...
	if(swCurrentDevType == devTypeDef_fans){
		...
		switch(devParam_fans.opreatAct){
			...
			case fansOpreatAct_firstGear:
				statusTemp = devFanCfgParam.subdivDetailGear_1;
				break;
			
			case fansOpreatAct_secondGear:
				statusTemp = devFanCfgParam.subdivDetailGear_2;
				break;
			
			case fansOpreatAct_thirdGear:
				statusTemp = devFanCfgParam.subdivDetailGear_3;
				break;
