...
static const stt_solarSysManagerCtrlOperateParam devSolarSysManagerCtrlOperateParam_defInit = {
 ...
 static bool SSMR_confineDevListRefDelay_en = false;
...
//		printf("takeOverCtrl node[%02X%02X%02X%02X%02X%02X] not found.\n", MAC2STR(dstMac));
 ...
 static void devDriverBussiness_solarSysManager_debugInfoPrint(void){

	stt_listAttrSolarSysManager *pAbove = NULL;
	stt_listAttrSolarSysManager *pFollow = NULL;
	uint16_t count = 0;

	printf(">>>solarSysManager debug info - \n");
	nvsDataOpreation_solarSysManagerDevList_generateToListFromTagetList(listHead_SSMRdevCtrlUnitManageTemp,
																		...
																		(1 << 0) | (1 << 1) | (1 << 2));
	pAbove = listHead_SSMRdevCtrlUnitManageTemp;
	while(pAbove->next != NULL){
		pFollow = pAbove;
		pAbove	= pFollow->next;
		count ++;
		printf("dev%d: mac:%02x%02x%02x%02x%02x%02x, state:%02X;\n", count, MAC2STR(pAbove->paramCtrlUnit.unitDevMac), pAbove->paramCtrlUnit.unitDevStateRcd);
	}
 ...
 void devDriverBussiness_solarSysManager_confineDevListRefDelay(void){
	 ...
	 SSMR_confineDevListRefDelay_en = true;
 }
...
//		devDriverBussiness_solarSysManager_bussinessDataReq(pAbove->paramCtrlUnit.unitDevMac, SSMR_voltageNormal_FLG, differAct_firstTrigFlg, false);
 ...
 void devDriverBussiness_solarSysManager_runningDetectLoop(void){
	...
	static stt_timerLoop confineDevListRefLooper = {(const uint16_t)(90.0F / SOLARSYSMANAGER_RUNNINGDETECT_PERIODLOOP_TIME), 0};
	static stt_timerLoop dbgLooper = {(const uint16_t)(3.0F / SOLARSYSMANAGER_RUNNINGDETECT_PERIODLOOP_TIME), 0};
	static stt_timerLoop tmrScrRefresh = {(const uint16_t)(120.0F / SOLARSYSMANAGER_RUNNINGDETECT_PERIODLOOP_TIME), 0};
	...
	if(dbgLooper.loopCounter < dbgLooper.loopPeriod)dbgLooper.loopCounter ++;
	else{
		dbgLooper.loopCounter = 0;
		// devDriverBussiness_solarSysManager_debugInfoPrint();
	}

//	printf("counter listGet:%d.\n", counter);

	if(true == SSMR_confineDevListRefDelay_en){
		SSMR_confineDevListRefDelay_en = false;
		confineDevListRefLooper.loopCounter = 0;
	}
	// printf(">>>SSMR confineDevList refresh counter:%d.\n", confineDevListRefLooper.loopCounter);
	if(confineDevListRefLooper.loopCounter < confineDevListRefLooper.loopPeriod)confineDevListRefLooper.loopCounter ++;
	else{ //每90s获取一次列表
		confineDevListRefLooper.loopCounter = 0;
