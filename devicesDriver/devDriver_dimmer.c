...
static stt_devDimmer_paramCfg devDimmerCfgParam = {
	.brightnessBaseVar = 0,
};
...
void devDriverBussiness_dimmerSwitch_cfgParamGet(stt_devDimmer_paramCfg *param){

	memcpy(param, &devDimmerCfgParam, sizeof(stt_devDimmer_paramCfg));
}

void devDriverBussiness_dimmerSwitch_cfgParamSet(stt_devDimmer_paramCfg *param, bool nvsRecord_IF){

	memcpy(&devDimmerCfgParam, param, sizeof(stt_devDimmer_paramCfg));

	if(nvsRecord_IF){
		devSystemInfoLocalRecord_save(saveObj_devDimmer_cfgParam, &devDimmerCfgParam);
	}
}
...
void devDriverBussiness_dimmerSwitch_periphStatusReales(stt_devDataPonitTypedef *param){
	...
	const float devPointVarMax = 100.0f;
	devTypeDef_enum swCurrentDevType = currentDev_typeGet();
	...
	if(swCurrentDevType == devTypeDef_dimmer){
		...
		float statusTemp_f = 0.0F;

		memcpy(&statusTemp, param, sizeof(uint8_t));
		if(0 != statusTemp){
			statusTemp_f = (float)devDimmerCfgParam.brightnessBaseVar + (devPointVarMax - (float)devDimmerCfgParam.brightnessBaseVar) / 100.0f * (float)statusTemp;
			statusTemp = (uint8_t)statusTemp_f;
		}

		// printf("adj_bt:%d, base_bt:%d.\n", statusTemp, devDimmerCfgParam.brightnessBaseVar);
