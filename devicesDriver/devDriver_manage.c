...
bool devTempDispMethod_unit_get(void){

	bool res = false;

	(deviceStatusRecordIF_Flg.devTempDispMethod)?
		(res = true):
		(res = false);
...
void devDriverApp_responseAtionTrig_delay(void){ //数据点触发延时响应动作，避免高频动作
	...
	devDetailInfoUploadTrig(); //细节数据同步上报触发
	...
	if(devType_rcv == currentDev_typeGet()){
		...
		switch(currentDev_typeGet()){ //根据设备类型进行扩展数据设置操作
			...
			case devTypeDef_heater:{
				...
				stt_msgDats_devHeaterDriver sptr_msgQ_devHeaterDriver = {
					.msgType = msgType_devHeaterDriver_cutomGearTimeChg,
				};
				...
				xQueueSend(msgQh_devHeaterDriver, &sptr_msgQ_devHeaterDriver, 20 / portTICK_RATE_MS); //通知刷新界面
...
//					uint8_t dataDecimal_prt = functionFloatGetDecimalW2Bit(dataHandle_temp);
					...
					stt_solarSysManagerCtrlOperateParam devRunningParam = {0};

					devDriverBussiness_solarSysManager_ctrlRunParamGet(&devRunningParam);
					(0 == devRunningParam.voltageTurnOffStageA_en)?
						(paramTemp[1] = 0):(paramTemp[1] = (uint8_t)devRunningParam.voltageTurnOffStage_A);
					paramTemp[2] = (uint8_t)devRunningParam.voltageTurnOn;
					paramTemp[3] = (uint8_t)devRunningParam.temperature_alarm;
					
					// functionDebugDptrToHex_printf("pxGet", paramTemp, 4);
