...
//					lv_img_set_src(objFigure_ptr[0], usrAppHomepageBkPic_dataGet());
				...
				case bGroudImg_objInsert_figureD:{scrHeaderSysInfo_baseColor = LV_COLOR_HEX(0x1d1d1d);}break;
...
static void pageHeader_infoRefreshLoop(void){
	...
	if(devRunningTimeFromPowerUp_couter < 5)return;
...
//								printf("watch point_C.\n");
								...
								if(NULL == preload_driverCalmDown_devScenario){
									...
									(devStatusDispMethod_landscapeIf_get())?
										(lv_obj_set_size(preload_driverCalmDown_devScenario, 35, 35)):
										(lv_obj_set_size(preload_driverCalmDown_devScenario, 45, 45));
				...
				case devTypeDef_voltageSensor:{
					...
					if(voltStateFlg_rcd != devDriverBussiness_SSMR_devVoltState_get()){
						...
						(voltStateFlg_rcd)?
							...
							(styleImage_solarSysManager_voltIcon->image.color = LV_COLOR_MAKE(255,  32, 64));
					...
					if(tempStateFlg_rcd != devDriverBussiness_SSMR_devTempState_get()){
						...
						(tempStateFlg_rcd)?
							...
							(styleImage_solarSysManager_tempIcon->image.color = LV_COLOR_MAKE(255,  32, 64));
						lv_obj_refresh_style(imgObj_solarSysManager_temprature);
					}
					
					if(false == devDriverBussiness_SSMR_devVoltState_get()){ //电压告警
						...
						switch(devDriverBussiness_SSMR_devVoltageCurStage_get()){
							case ssmrCtrlStage_voltageLow_A:
								...
								if(1 == ctrlParam.voltageTurnOffStageB_en)
							...
							case ssmrCtrlStage_voltageLow_C:
								strcpy(colorStr, "FF2020");
								break;
							default:strcpy(colorStr, "B0E61D");break;
						}

						(localCounter < tipsPeriod)?
							(localCounter ++):
							(localCounter = 0);
						
						(localCounter > (tipsPeriod / 2))?
							(dispFlg = true):
							(dispFlg = false);
						if(dispFlg_rcd != dispFlg){
							...
							if(dispFlg)sprintf(tempInfomationDisp_volt, "#%s %d.%02dV#", colorStr, functionFloatGetInteger(volatageVal_cur), functionFloatGetDecimalW2Bit(volatageVal_cur));
							else sprintf(tempInfomationDisp_volt, "   ");
...
//							lv_obj_set_top(imageObj_solarSysManager_meansureUnit, true);
					...
					if(false == devDriverBussiness_SSMR_devTempState_get()){ //温度告警
						...
						if(dispFlg_rcd != dispFlg){
							...
							if(dispFlg)sprintf(tempInfomationDisp_temp, "#FF2020 %d.%02dC#", functionFloatGetInteger(tempratureVal_cur), functionFloatGetDecimalW2Bit(tempratureVal_cur));
							else sprintf(tempInfomationDisp_volt, "   ");
...
//				}
		...
		case bussinessType_menuPageLinkageConfig:{}break;
...
void pageHome_buttonMain_imageRefresh(bool freshNoRecord){ //界面切换时调用一次，数据点设置时调用一次
	...
	if(devRunningTimeFromPowerUp_couter < 10)return; //避免控件未创建就被使用
...
//						devDriverBussiness_solarSysManager_exDevParamGet(&exDataTemp_SSMR_opParam);
						...
						(ctrlRunningTemp_SSMR_opParam.voltageTurnOffStageA_en)?
							(sprintf(voltageStrTemp[0], "%.02fV", ctrlRunningTemp_SSMR_opParam.voltageTurnOffStage_A)):
							(strcpy(voltageStrTemp[0], "DISABLE"));
						(ctrlRunningTemp_SSMR_opParam.voltageTurnOffStageB_en)?
							(sprintf(voltageStrTemp[1], "%.02fV", ctrlRunningTemp_SSMR_opParam.voltageTurnOffStage_B)):
							(strcpy(voltageStrTemp[1], "DISABLE"));
						(ctrlRunningTemp_SSMR_opParam.voltageTurnOffStageC_en)?
							(sprintf(voltageStrTemp[2], "%.02fV", ctrlRunningTemp_SSMR_opParam.voltageTurnOffStage_C)):
...
//	lv_obj_set_style(textBtn_toilet, styleBtn_Text);
	...
	(devStatusDispMethod_landscapeIf_get())?
		(lv_obj_set_size(textBtn_meeting, 168, 40)):
		(lv_obj_set_size(textBtn_meeting, 128, 40));
	(devStatusDispMethod_landscapeIf_get())?
		(lv_obj_set_size(textBtn_sleeping, 168, 40)):
		(lv_obj_set_size(textBtn_sleeping, 128, 40));
	(devStatusDispMethod_landscapeIf_get())?
		(lv_obj_set_size(textBtn_toilet, 168, 40)):
		(lv_obj_set_size(textBtn_toilet, 128, 40));
	lv_label_set_text(textBtn_meeting, (const char*)dataTextObjDisp_temp.dataBtnTextDisp[0]);
	lv_label_set_text(textBtn_sleeping, (const char*)dataTextObjDisp_temp.dataBtnTextDisp[1]);
	lv_label_set_text(textBtn_toilet, (const char*)dataTextObjDisp_temp.dataBtnTextDisp[2]);
	lv_obj_set_protect(textBtn_meeting, LV_PROTECT_POS);
	...
	(devStatusDispMethod_landscapeIf_get())?
		(lv_obj_align(textBtn_meeting, NULL, LV_ALIGN_IN_LEFT_MID, 75, 5)):
		(lv_obj_align(textBtn_meeting, NULL, LV_ALIGN_IN_LEFT_MID, 71, 24));
	(devStatusDispMethod_landscapeIf_get())?
		(lv_obj_align(textBtn_sleeping, NULL, LV_ALIGN_IN_LEFT_MID, 75, 5)):
		(lv_obj_align(textBtn_sleeping, NULL, LV_ALIGN_IN_LEFT_MID, 71, 24));
	(devStatusDispMethod_landscapeIf_get())?
		(lv_obj_align(textBtn_toilet, NULL, LV_ALIGN_IN_LEFT_MID, 75, 5)):
		(lv_obj_align(textBtn_toilet, NULL, LV_ALIGN_IN_LEFT_MID, 71, 24));
...
static void local_guiHomeBussiness_solarSysManager(lv_obj_t * obj_Parent){
	...
	devStatusDispMethod_landscapeIf_get()?
		(lv_obj_align(imgObj_solarSysManager_voltage, NULL, LV_ALIGN_IN_TOP_LEFT, 60, 50)):
		(lv_obj_align(imgObj_solarSysManager_voltage, NULL, LV_ALIGN_IN_TOP_LEFT, 15, 75));
	...
	devStatusDispMethod_landscapeIf_get()?
		(lv_obj_align(imgObj_solarSysManager_temprature, imgObj_solarSysManager_voltage, LV_ALIGN_OUT_BOTTOM_MID, 0, 40)):
		(lv_obj_align(imgObj_solarSysManager_temprature, imgObj_solarSysManager_voltage, LV_ALIGN_OUT_BOTTOM_MID, 0, 60));
	...
	(devParamCtrlRunning_temp.voltageTurnOffStageA_en)?
		(sprintf(voltageStrTemp[0], "%.02fV", devParamCtrlRunning_temp.voltageTurnOffStage_A)):
		(strcpy(voltageStrTemp[0], "DISABLE"));
	memset(voltageStrTemp[1], 0, sizeof(voltageStrTemp[1]));
	(devParamCtrlRunning_temp.voltageTurnOffStageB_en)?
		(sprintf(voltageStrTemp[1], "%.02fV", devParamCtrlRunning_temp.voltageTurnOffStage_B)):
		(strcpy(voltageStrTemp[1], "DISABLE"));
	memset(voltageStrTemp[2], 0, sizeof(voltageStrTemp[2]));
	(devParamCtrlRunning_temp.voltageTurnOffStageC_en)?
		(sprintf(voltageStrTemp[2], "%.02fV", devParamCtrlRunning_temp.voltageTurnOffStage_C)):
		(strcpy(voltageStrTemp[2], "DISABLE"));
	memset(strTemp_dispInfo, 0, sizeof(strTemp_dispInfo));
...
//	lv_obj_animate(btn_homeMenu, LV_ANIM_FLOAT_LEFT, 300, 100, NULL);
	...
	if(1 == devStatusRecordFlg_temp.devListManager_En){
		...
		for(loop = 0; loop < DEVICE_TYPE_LIST_NUM; loop ++){
			...
			if(devTypeDef_null == guiHomeBussiness_tab[loop].currentDevType)break;
			else
...
static void lvglUsrApp_newVersionRunningConfirm(void){
	...
	static uint16_t realesTipsShowTout_counter = timecof_1sPeriod * 600; //提示显示超时：600s / 10min
	...
	while(true == uiBlockFlg_newVersionRunningConfirm){
		...
		if(realesTipsShowTout_counter > 0){
			...
			if((0 == (realesTipsShowTout_counter % timecof_1sPeriod)) &&
			   ((realesTipsShowTout_counter / timecof_1sPeriod) < 10)){
...
//	lv_obj_animate(objLabelRef_epidDeaths,	 LV_ANIM_FLOAT_LEFT,  200, 50, NULL);
 ...
 static void lvglUsrApp_screensaverNormal_A_allObjRefresh(void){
	...
	switch(currentDev_typeGet()){
		...
		case devTypeDef_voltageSensor:{}break;
#endif
