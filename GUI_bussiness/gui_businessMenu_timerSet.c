...
static void lvGuiMenuTimerSet_styleMemoryDeinit(void);
...
//	lvGuiMenuTimerSet_styleMemoryDeinit();
...
//		sprintf(strTemp, labelStrGet_bySysLanguage(tls_mTimer_noRepeat));
	}
	else
	{
...
//	printf("timer dimmer set trigVal:%d.\n", datapointParamSet_temp.devType_dimmer.devDimmer_brightnessVal);
	...
	printf("timer%dParamSave trig, upVal:%d, en:%d, weekbit:%d, hour:%d, minute:%d.\n", tmOpNum,
...
static void lvGui_bMenuSet_B_pageReaptSet_creat(uint8_t currentWeekHoldBit){
	...
	lv_coord_t cbWeekNumPos_offset_x = 0;

	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:cbWeekNumPos_offset_x = -22;break;
		case languageCyId_Portuguese:{
				(devStatusDispMethod_landscapeIf_get())?
					(cbWeekNumPos_offset_x = 22):
					(cbWeekNumPos_offset_x = 12);
			}break;
		default:cbWeekNumPos_offset_x = -12;break;}
	...
	styleTextPageTitle_reaptSet->text.font = fontLabelGet_bySysLanguage(4);
...
//	lv_label_set_text(objpageLabel_title, "Repeat cycle");
	lv_label_set_text(objpageLabel_title, labelStrGet_bySysLanguage(tls_mTimer_repeatCycle));
	...
	stylePageCb_reaptSet->text.font = fontLabelGet_bySysLanguage(2);
...
//	lv_cb_set_text(objCb_dataSet_mon, " Monday");
	lv_cb_set_text(objCb_dataSet_mon, labelStrGet_bySysLanguage(tls_mTimer_Monday));
	...
	(devStatusDispMethod_landscapeIf_get())?
		...
		(lv_obj_align(objCb_dataSet_mon, objpageLabel_title, LV_ALIGN_OUT_BOTTOM_LEFT, cbWeekNumPos_offset_x, 7));
	objCb_dataSet_tues = lv_cb_create(objpage_reaptSet, NULL);
//	lv_cb_set_text(objCb_dataSet_tues, " Tuesday");
	lv_cb_set_text(objCb_dataSet_tues, labelStrGet_bySysLanguage(tls_mTimer_Tuesday));
...
//	lv_cb_set_text(objCb_dataSet_wed, " Wednesday");
	lv_cb_set_text(objCb_dataSet_wed, labelStrGet_bySysLanguage(tls_mTimer_Wednesday));
...
//	lv_cb_set_text(objCb_dataSet_thur, " Thursday");
	lv_cb_set_text(objCb_dataSet_thur, labelStrGet_bySysLanguage(tls_mTimer_Thursday));
...
//	lv_cb_set_text(objCb_dataSet_fri, " Friday");
	lv_cb_set_text(objCb_dataSet_fri, labelStrGet_bySysLanguage(tls_mTimer_Friday));
...
//	lv_cb_set_text(objCb_dataSet_sat, " Saturday");
	lv_cb_set_text(objCb_dataSet_sat, labelStrGet_bySysLanguage(tls_mTimer_Saturday));
...
//	lv_cb_set_text(objCb_dataSet_sun, " Sunday");
	lv_cb_set_text(objCb_dataSet_sun, labelStrGet_bySysLanguage(tls_mTimer_Sunday));
	...
	stylePageBtn_reaptSet->text.font = fontLabelGet_bySysLanguage(2);
...
//	lv_label_set_text(objpageBtnLabel_confirm, "Confirm");
	lv_label_set_text(objpageBtnLabel_confirm, labelStrGet_bySysLanguage(tls_confirm));
...
//	lv_label_set_text(objpageBtnLabel_cancel, "Cancel");
	lv_label_set_text(objpageBtnLabel_cancel, labelStrGet_bySysLanguage(tls_cancel));
...
static void lvGuiMenuTimerSet_styleMemoryInitialization(void){
	...
	styleRoller_timeSet_bg 		= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleRoller_timeSet_sel 	= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleSw_devStatus_bg 		= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleSw_devStatus_indic 	= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleSw_devStatus_knobOn 	= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleSw_devStatus_knobOff 	= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleBk_objBground 			= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleBtn_specialTransparent_rel = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleBtn_specialTransparent_pr = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleBtn_specialTransparent_tgl_rel = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleBtn_specialTransparent_tgl_pr = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	stylePage_reaptSet 			= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	stylePageCb_reaptSet 		= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleTextPageTitle_reaptSet = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	stylePageBtn_reaptSet 		= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleText_menuLevel_A 		= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleText_menuSetChoiceTitle = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleText_menuDevStatus 	= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleText_menuTimeSet 		= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleText_menuReaptSet 		= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleText_menuReaptRem 		= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	
	stylePage_timerSetPageA 	= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleBtn_timerSetPageA 		= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleLabel_btnTimerSetPageA_timerInfo = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleLabel_btnTimerSetPageA_reaptInfo = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
...
static void lvGuiMenuTimerSet_styleMemoryDeinit(void){
	...
	LV_STYLE_MEM_FREE(styleRoller_timeSet_bg);
	LV_STYLE_MEM_FREE(styleRoller_timeSet_sel);
	LV_STYLE_MEM_FREE(styleSw_devStatus_bg);
	LV_STYLE_MEM_FREE(styleSw_devStatus_indic);
	LV_STYLE_MEM_FREE(styleSw_devStatus_knobOn);
	LV_STYLE_MEM_FREE(styleSw_devStatus_knobOff);
	LV_STYLE_MEM_FREE(styleBk_objBground);
	LV_STYLE_MEM_FREE(styleBtn_specialTransparent_rel);
	LV_STYLE_MEM_FREE(styleBtn_specialTransparent_pr);
	LV_STYLE_MEM_FREE(styleBtn_specialTransparent_tgl_rel);
	LV_STYLE_MEM_FREE(styleBtn_specialTransparent_tgl_pr);
	LV_STYLE_MEM_FREE(stylePage_reaptSet);
	LV_STYLE_MEM_FREE(stylePageCb_reaptSet);
	LV_STYLE_MEM_FREE(styleTextPageTitle_reaptSet);
	LV_STYLE_MEM_FREE(stylePageBtn_reaptSet);
	LV_STYLE_MEM_FREE(styleText_menuLevel_A);
	LV_STYLE_MEM_FREE(styleText_menuSetChoiceTitle);
	LV_STYLE_MEM_FREE(styleText_menuDevStatus);
	LV_STYLE_MEM_FREE(styleText_menuTimeSet);
	LV_STYLE_MEM_FREE(styleText_menuReaptSet);
	LV_STYLE_MEM_FREE(styleText_menuReaptRem);
	LV_STYLE_MEM_FREE(stylePage_timerSetPageA);
	LV_STYLE_MEM_FREE(styleBtn_timerSetPageA);
	LV_STYLE_MEM_FREE(styleLabel_btnTimerSetPageA_timerInfo);
	LV_STYLE_MEM_FREE(styleLabel_btnTimerSetPageA_reaptInfo);
...
static void lvGui_businessMenu_timerSetPageA(lv_obj_t * obj_Parent){
	...
	styleLabel_btnTimerSetPageA_reaptInfo->text.font = &lv_font_dejavu_15;
...
//	lv_label_set_text(objText_menuCurrentTitle, "Timer setting");
	lv_label_set_text(objText_menuCurrentTitle, labelStrGet_bySysLanguage(tls_mTimer_timerSetting));
	if(languageCyId_Arabic == devLanguageCountry_identifyGet()){
		lv_obj_align(objText_menuCurrentTitle, NULL, LV_ALIGN_IN_BOTTOM_MID, -35, -7);
	}else{
...
//	printf("usrApp timer:%d, set opreat.\n", timerUnit_num);
	...
	styleLabel_SetInfoTitle_opreation->text.font = fontLabelGet_bySysLanguage(2);
	...
	switch(devLanguageCountry_identifyGet()){
	case languageCyId_Portuguese:lv_obj_align(btnTitle_setInfo_save, NULL, LV_ALIGN_IN_BOTTOM_RIGHT, -48, 0);break;
	default:lv_obj_align(btnTitle_setInfo_save, NULL, LV_ALIGN_IN_BOTTOM_RIGHT, 7, 0);break;}
...
//	lv_label_set_text(labelTitle_setInfo_save, "#4444FF Save#");
	...
	lv_label_set_text(labelTitle_setInfo_save, (const char*)strTemp);
...
//	lv_label_set_text(labelTitle_setInfo_cancel, "#4444FF Cancel#");
	...
	lv_label_set_text(labelTitle_setInfo_cancel, (const char*)strTemp);
	...
	styleText_menuSetChoiceTitle->text.font = fontLabelGet_bySysLanguage(2);
...
//	lv_label_set_text(objLabel_setChoiceTitle_switchSet, "Switch setting:");
	lv_label_set_text(objLabel_setChoiceTitle_switchSet, labelStrGet_bySysLanguage(tls_mTimer_switchSetting));
	lv_obj_set_protect(objLabel_setChoiceTitle_switchSet, LV_PROTECT_POS);
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:
			lv_obj_align(objLabel_setChoiceTitle_switchSet, objLine_timeSet_limit_A, LV_ALIGN_OUT_BOTTOM_RIGHT, -10, 9);
			break;
		default:
...
//	lv_label_set_text(objLabel_setChoiceTitle_reaptSet, "Repeat setting:");
	lv_label_set_text(objLabel_setChoiceTitle_reaptSet, labelStrGet_bySysLanguage(tls_mTimer_repeatSetting));
	lv_obj_set_protect(objLabel_setChoiceTitle_reaptSet, LV_PROTECT_POS);
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:
			lv_obj_align(objLabel_setChoiceTitle_reaptSet, objLine_timeSet_limit_B, LV_ALIGN_OUT_BOTTOM_RIGHT, -10, 9);
			break;
		default:
...
//	lv_label_set_text(objLabel_setChoiceTitle_timeSet, "Time setting:");
	lv_label_set_text(objLabel_setChoiceTitle_timeSet, labelStrGet_bySysLanguage(tls_mTimer_timeSetting));
	lv_obj_set_protect(objLabel_setChoiceTitle_timeSet, LV_PROTECT_POS);
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:
			lv_obj_align(objLabel_setChoiceTitle_timeSet, objLine_timeSet_limit_C, LV_ALIGN_OUT_BOTTOM_RIGHT, -10, 9);
			break;
		default:
...
//	lv_obj_align(objLabel_devStatus_C, objLabel_devStatus_B, LV_ALIGN_CENTER, 0, 35);
	...
	styleText_menuReaptSet->text.font = fontLabelGet_bySysLanguage(2);
...
//	lv_label_set_text(objLabel_reaptSet, "Repeat cycle");
	lv_label_set_text(objLabel_reaptSet, labelStrGet_bySysLanguage(tls_mTimer_repeatCycle));
...
//	styleText_menuReaptRem->text.font = fontLabelGet_bySysLanguage(0);
...
//		sprintf(strTemp, "No repeat");
	...
	styleText_menuTimeSet->text.font = fontLabelGet_bySysLanguage(0);
...
//	lv_label_set_text(objLabel_timeSet_hour, "hour");
	lv_label_set_text(objLabel_timeSet_hour, labelStrGet_bySysLanguage(tls_hour));
	...
	lv_obj_align(objLabel_timeSet_hour, objRoller_timeSet_hour, LV_ALIGN_CENTER, coordOffset_cLanguage_x, 2);
...
//	lv_label_set_text(objLabel_timeSet_minute, "minute");
	lv_label_set_text(objLabel_timeSet_minute, labelStrGet_bySysLanguage(tls_minute));
	lv_obj_set_protect(objLabel_timeSet_minute, LV_PROTECT_POS);
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:coordOffset_cLanguage_x = 28;break;
		default:coordOffset_cLanguage_x = 38;break;}
	lv_obj_align(objLabel_timeSet_minute, objRoller_timeSet_minute, LV_ALIGN_CENTER, coordOffset_cLanguage_x, 0);
