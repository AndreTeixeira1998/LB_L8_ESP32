...
static lv_obj_t *labelSysInfo_parentSSID = NULL;
static lv_obj_t *labelSysInfo_routerName = NULL;
...
void lvGuiOther_devInfoRefresh(void){
	...
	(devStatusRecordFlg_temp.devTempDispMethod)?
		(sprintf(textDisp_temp, "%s \n  #408080 %s%.02f# \"F", labelStrGet_bySysLanguage(tls_mSysInfo_deviceTemperature), textTabAlign, varConverionCelsius2Fahrenheit_f(devDriverBussiness_temperatureMeasure_get()))):
	...
	memset(textDisp_temp, 0, sizeof(char) * textTempSize);
	sprintf(textDisp_temp, "%s \n  #B5E61D %s%s#", labelStrGet_bySysLanguage(tls_mSysInfo_parentSSID), textTabAlign, wifi_config.sta.ssid);
	lv_label_set_text(labelSysInfo_parentSSID, textDisp_temp);

	memset(textDisp_temp, 0, sizeof(char) * textTempSize);
	sprintf(textDisp_temp, "%s \n  #B5E61D %s%s#", labelStrGet_bySysLanguage(tls_mSysInfo_routerName), textTabAlign, routerCfgInfo_temp.routerInfo_ssid);
	lv_label_set_text(labelSysInfo_routerName, textDisp_temp);
...
//	lv_obj_set_top(menuBtnChoIcon_fun_back, true);
	...
	if(devStatusDispMethod_landscapeIf_get()){
		...
		lv_page_set_scrl_height(page_sysInfo, 1380); 
	}
	else{
		lv_page_set_scrl_width(page_sysInfo, 220); //scrl尺寸必须大于set size尺寸 才可以进行拖拽
		lv_page_set_scrl_height(page_sysInfo, 1440); //scrl尺寸必须大于set size尺寸 才可以进行拖拽
	}
	...
	labelSysInfo_parentSSID = lv_label_create(page_sysInfo, 	labelSysInfo_devTime);
	lv_obj_align(labelSysInfo_parentSSID, 		page_sysInfo, coordOffset_alignLanguage_temp, textHorizontalDistance, objLayoutCursor += textVerticalDistance);
	labelSysInfo_routerName = lv_label_create(page_sysInfo, 	labelSysInfo_devTime);
	lv_obj_align(labelSysInfo_routerName, 		page_sysInfo, coordOffset_alignLanguage_temp, textHorizontalDistance, objLayoutCursor += textVerticalDistance);
	...
	lv_obj_align(labelSysInfo_devSunScale, 	page_sysInfo, coordOffset_alignLanguage_temp, textHorizontalDistance, objLayoutCursor += textVerticalDistance + 30);
