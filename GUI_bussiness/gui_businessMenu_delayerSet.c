...
static lv_style_t *styleTabview_gmdyTimeSet_bg = NULL;
static lv_style_t *styleTabview_gmdyTimeSet_indic = NULL;
static lv_style_t *styleTabview_gmdyTimeSet_btn_bg = NULL;
static lv_style_t *styleTabview_gmdyTimeSet_btn_pr = NULL;
static lv_style_t *styleTabview_gmdyTimeSet_btn_rel = NULL;
static lv_style_t *styleTabview_gmdyTimeSet_btn_tgl_pr = NULL;
static lv_style_t *styleTabview_gmdyTimeSet_btn_tgl_rel = NULL;

static lv_style_t *styleRoller_gmdyTimeSet_bg = NULL;
static lv_style_t *styleRoller_gmdyTimeSet_sel = NULL;
static lv_style_t *styleLabel_gmdyTimeSet_rollerIns = NULL;
static lv_style_t *styleLabel_gmdyTimeSet_title = NULL;
static lv_style_t *styleLabel_gmdyTimeSet_funSave = NULL;
static lv_style_t *styleBtn_gmdyTimeSet_funSave = NULL;
...
static lv_style_t *stylePageCb_gModeRelaySel = NULL;
...
static lv_obj_t *objLine_greenModeSet_limit_C = NULL;
...
static lv_obj_t *objLabel_greenModeSet_titleRelaySel = NULL;
static lv_obj_t *objLabel_greenModeSet_titleSwStatusNormal = NULL;
static lv_obj_t *objLabel_greenModeSet_titleSwStatusRef_NC = NULL;
static lv_obj_t *objLabel_greenModeSet_titleSwStatusRef_NO = NULL;
...
static lv_obj_t *objSw_greenModeSet_statusNormal = NULL;
static lv_obj_t *cbGMode_relaySelect[3] = {NULL};
...
static bool gModeObjAnimDone_flg = false;

static void lvGui_delayerSet_gModeOpParamObj(bool creatIf);
static void lvGuiMenuDelayer_styleMemoryDeinit(void);
...
//	lvGuiMenuDelayer_styleMemoryDeinit();

	objLabel_greenModeSet_titleSwStatusNormal = NULL;
	gModeObjAnimDone_flg = false;
...
static lv_res_t funCb_mboxBtnActionClick_paramSaveSuccess(lv_obj_t * mbox, const char * txt){
	...
	lv_obj_set_click(objBtn_delayTrigSet_save, true);
	lv_obj_set_click(objBtn_greenModeSet_save, true);
...
static void lvGuiDelayerSet_mboxCreat_paramSaveSuccess(void){
	...
	mboxBtnKbCall_paramSaveSuccess = lv_mbox_create(objTabv_gmdyOpMenu, NULL);
	lv_obj_set_protect(mboxBtnKbCall_paramSaveSuccess, LV_PROTECT_POS);
	lv_obj_align(mboxBtnKbCall_paramSaveSuccess, lv_scr_act(), LV_ALIGN_CENTER, -35, -20);
	lv_obj_set_width(mboxBtnKbCall_paramSaveSuccess, 230);
	lv_mbox_set_text(mboxBtnKbCall_paramSaveSuccess, dialogWordsStrGet_bySysLanguage(dialogTips_opetSuccess));
	lv_mbox_add_btns(mboxBtnKbCall_paramSaveSuccess, dialogSelTabGet_bySysLanguage(dialogSelTab_ok), NULL);
	...
	lv_obj_set_click(objBtn_delayTrigSet_save, false);
	lv_obj_set_click(objBtn_greenModeSet_save, false);
}
...
static lv_res_t funCb_swOp_greenModeSet_runSetUp(lv_obj_t *sw){
	...
	if(lv_sw_get_state(sw)){
		...
		if(0 == greenMode_timeCount){ //不能为零

			param_greenModeTimeSet.second = 5;
		}

		lvGui_delayerSet_gModeOpParamObj(1);
	}
	else
	{
		...
		lvGui_delayerSet_gModeOpParamObj(0);
	}
...
static lv_res_t funCb_swOp_greenModeSet_statusNormal(lv_obj_t *sw){

	stt_gModeOpFunc opParamTemp = {0};

	usrAppParamGet_devGreenMode_actOption(&opParamTemp);

	lv_sw_get_state(sw)?
		(opParamTemp.opStatusNormal = 1):
		(opParamTemp.opStatusNormal = 0);

	usrAppParamSet_devGreenMode_actOption(&opParamTemp, true);

	return LV_RES_OK;
}

static lv_res_t funCb_cbOpreat_gModeRelaySel(lv_obj_t *cb){
...
static void lvGui_delayerSet_gModeOpParamObj(bool creatIf){
	...
	stt_gModeOpFunc gModeOpFuncParam_temp = {0};

	usrAppParamGet_devGreenMode_actOption(&gModeOpFuncParam_temp);

	if(creatIf){

		if(objLabel_greenModeSet_titleSwStatusNormal == NULL){
			...
			switch(currentDev_typeGet()){
				...
				case devTypeDef_mulitSwThreeBit:{
					...
					objLabel_greenModeSet_titleSwStatusNormal = lv_label_create(objTab_greenModeSet, NULL);
//					lv_label_set_text(objLabel_greenModeSet_titleSwStatusNormal, "Status normal:");
					lv_label_set_text(objLabel_greenModeSet_titleSwStatusNormal, labelStrGet_bySysLanguage(tls_mDelayer_statusNormal));
					lv_obj_set_protect(objLabel_greenModeSet_titleSwStatusNormal, LV_PROTECT_POS);
					switch(devLanguageCountry_identifyGet()){
						case languageCyId_Arabic:lv_obj_align(objLabel_greenModeSet_titleSwStatusNormal, objLabel_greenModeSet_titleSwValTrig, LV_ALIGN_OUT_BOTTOM_RIGHT, -71, 60);break;
						default:lv_obj_align(objLabel_greenModeSet_titleSwStatusNormal, objLabel_greenModeSet_titleSwValTrig, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 60);break;}
					lv_obj_set_style(objLabel_greenModeSet_titleSwStatusNormal, styleLabel_gmdyTimeSet_title);
			
					objSw_greenModeSet_statusNormal = lv_sw_create(objTab_greenModeSet, NULL);
					lv_sw_set_action(objSw_greenModeSet_statusNormal, funCb_swOp_greenModeSet_statusNormal);
					lv_obj_set_size(objSw_greenModeSet_statusNormal, 40, 20);
					lv_obj_align(objSw_greenModeSet_statusNormal, objLine_greenModeSet_limit_B, LV_ALIGN_OUT_BOTTOM_MID, 0, 115);
					(gModeOpFuncParam_temp.opStatusNormal)?
						(lv_sw_on(objSw_greenModeSet_statusNormal)):
						(lv_sw_off(objSw_greenModeSet_statusNormal));
			
					objLabel_greenModeSet_titleSwStatusRef_NO = lv_label_create(objTab_greenModeSet, NULL);
					lv_label_set_text(objLabel_greenModeSet_titleSwStatusRef_NO, "#FFFF00 NC-#");
					lv_label_set_recolor(objLabel_greenModeSet_titleSwStatusRef_NO, true);
					lv_obj_set_protect(objLabel_greenModeSet_titleSwStatusRef_NO, LV_PROTECT_POS);
					lv_obj_align(objLabel_greenModeSet_titleSwStatusRef_NO, objSw_greenModeSet_statusNormal, LV_ALIGN_OUT_LEFT_MID, -10, 4);
					lv_obj_set_style(objLabel_greenModeSet_titleSwStatusRef_NO, styleLabel_gmdyTimeSet_title);
					objLabel_greenModeSet_titleSwStatusRef_NC = lv_label_create(objTab_greenModeSet, objLabel_greenModeSet_titleSwStatusRef_NO);
					lv_label_set_text(objLabel_greenModeSet_titleSwStatusRef_NC, "#FFFF00  -NO#");
					lv_obj_align(objLabel_greenModeSet_titleSwStatusRef_NC, objSw_greenModeSet_statusNormal, LV_ALIGN_OUT_RIGHT_MID, 10, 2);
			...
			switch(currentDev_typeGet()){
				...
				case devTypeDef_mulitSwOneBit:{
					...
					if(gModeObjAnimDone_flg == false){
					
						gModeObjAnimDone_flg = true;
						lv_page_focus(objTab_greenModeSet, objSw_greenModeSet_statusNormal, 1000);
					}
				...
				case devTypeDef_mulitSwTwoBit:{
					...
					objLine_greenModeSet_limit_C = lv_obj_create(objTab_greenModeSet, objLine_greenModeSet_limit_A);
					lv_obj_set_protect(objLine_greenModeSet_limit_C, LV_PROTECT_POS);
					lv_obj_align(objLine_greenModeSet_limit_C, objLine_greenModeSet_limit_B, LV_ALIGN_CENTER, 0, 160);
					
					objLabel_greenModeSet_titleRelaySel = lv_label_create(objTab_greenModeSet, NULL);
//					lv_label_set_text(objLabel_greenModeSet_titleRelaySel, "Action object:");
					lv_label_set_text(objLabel_greenModeSet_titleRelaySel, labelStrGet_bySysLanguage(tls_mDelayer_actionObj));
					lv_obj_set_protect(objLabel_greenModeSet_titleRelaySel, LV_PROTECT_POS);
					switch(devLanguageCountry_identifyGet()){
						case languageCyId_Arabic:lv_obj_align(objLabel_greenModeSet_titleRelaySel, objLine_greenModeSet_limit_C, LV_ALIGN_OUT_BOTTOM_RIGHT, -47, 2);break;
						default:lv_obj_align(objLabel_greenModeSet_titleRelaySel, objLine_greenModeSet_limit_C, LV_ALIGN_OUT_BOTTOM_LEFT, 3, 2);break;}
					lv_obj_set_style(objLabel_greenModeSet_titleRelaySel, styleLabel_gmdyTimeSet_title);

					cbGMode_relaySelect[0] = lv_cb_create(objTab_greenModeSet, NULL);
					lv_obj_set_size(cbGMode_relaySelect[0], 160, 15);
					lv_cb_set_text(cbGMode_relaySelect[0], " Relay1");
					//	switch(devLanguageCountry_identifyGet()){
					//		case languageCyId_Arabic:break;
					//		default:break;}
					lv_obj_align(cbGMode_relaySelect[0], objLine_greenModeSet_limit_C, LV_ALIGN_OUT_BOTTOM_MID, 0, 30);
					lv_cb_set_style(cbGMode_relaySelect[0], LV_CB_STYLE_BG, stylePageCb_gModeRelaySel);
					
					cbGMode_relaySelect[1] = lv_cb_create(objTab_greenModeSet, cbGMode_relaySelect[0]);
					lv_cb_set_text(cbGMode_relaySelect[1], " Relay2");
					lv_obj_align(cbGMode_relaySelect[1], cbGMode_relaySelect[0], LV_ALIGN_OUT_BOTTOM_LEFT, 0, 0);
			
					lv_obj_set_free_num(cbGMode_relaySelect[0], 0);
					lv_obj_set_free_num(cbGMode_relaySelect[1], 1);
					lv_cb_set_action(cbGMode_relaySelect[0], funCb_cbOpreat_gModeRelaySel);
					lv_cb_set_action(cbGMode_relaySelect[1], funCb_cbOpreat_gModeRelaySel);
			
					lv_cb_set_checked(cbGMode_relaySelect[0], gModeOpFuncParam_temp.opRelaySel & (1 << 0));
					lv_cb_set_checked(cbGMode_relaySelect[1], gModeOpFuncParam_temp.opRelaySel & (1 << 1));

					if(gModeObjAnimDone_flg == false){

						gModeObjAnimDone_flg = true;
						lv_page_focus(objTab_greenModeSet, cbGMode_relaySelect[0], 1000);
					}
...
//					lv_label_set_text(objLabel_greenModeSet_titleRelaySel, "Action object:");
					...
					cbGMode_relaySelect[2] = lv_cb_create(objTab_greenModeSet, cbGMode_relaySelect[0]);
					lv_cb_set_text(cbGMode_relaySelect[2], " Relay3");
					lv_obj_align(cbGMode_relaySelect[2], cbGMode_relaySelect[1], LV_ALIGN_OUT_BOTTOM_LEFT, 0, 0);
			
					lv_obj_set_free_num(cbGMode_relaySelect[0], 0);
					lv_obj_set_free_num(cbGMode_relaySelect[1], 1);
					lv_obj_set_free_num(cbGMode_relaySelect[2], 2);
					lv_cb_set_action(cbGMode_relaySelect[0], funCb_cbOpreat_gModeRelaySel);
					lv_cb_set_action(cbGMode_relaySelect[1], funCb_cbOpreat_gModeRelaySel);
					lv_cb_set_action(cbGMode_relaySelect[2], funCb_cbOpreat_gModeRelaySel);
			
					lv_cb_set_checked(cbGMode_relaySelect[0], gModeOpFuncParam_temp.opRelaySel & (1 << 0));
					lv_cb_set_checked(cbGMode_relaySelect[1], gModeOpFuncParam_temp.opRelaySel & (1 << 1));
					lv_cb_set_checked(cbGMode_relaySelect[2], gModeOpFuncParam_temp.opRelaySel & (1 << 2));
		...
		if(objLabel_greenModeSet_titleSwStatusNormal != NULL){
			...
			switch(currentDev_typeGet()){
				...
				case devTypeDef_mulitSwOneBit:{
					...
					lv_obj_del(objLabel_greenModeSet_titleSwStatusNormal);
					lv_obj_del(objLabel_greenModeSet_titleSwStatusRef_NC);
					lv_obj_del(objLabel_greenModeSet_titleSwStatusRef_NO);
					lv_obj_del(objSw_greenModeSet_statusNormal);

					objLabel_greenModeSet_titleSwStatusNormal = NULL;
					objLabel_greenModeSet_titleSwStatusRef_NC = NULL;
					objLabel_greenModeSet_titleSwStatusRef_NO = NULL;
					objSw_greenModeSet_statusNormal = NULL;
				...
				case devTypeDef_mulitSwThreeBit:{
					...
					lv_obj_del(objLine_greenModeSet_limit_C);
					lv_obj_del(objLabel_greenModeSet_titleSwStatusNormal);
					lv_obj_del(objLabel_greenModeSet_titleSwStatusRef_NC);
					lv_obj_del(objLabel_greenModeSet_titleSwStatusRef_NO);
					lv_obj_del(objLabel_greenModeSet_titleRelaySel);
					lv_obj_del(objSw_greenModeSet_statusNormal);

					objLine_greenModeSet_limit_C = NULL;
					objLabel_greenModeSet_titleSwStatusNormal = NULL;
					objLabel_greenModeSet_titleSwStatusRef_NC = NULL;
					objLabel_greenModeSet_titleSwStatusRef_NO = NULL;
					objLabel_greenModeSet_titleRelaySel = NULL;
					objSw_greenModeSet_statusNormal = NULL;
			...
			switch(currentDev_typeGet()){
				...
				case devTypeDef_mulitSwTwoBit:{
					...
					lv_obj_del(cbGMode_relaySelect[0]);
					lv_obj_del(cbGMode_relaySelect[1]);

					cbGMode_relaySelect[0] = NULL;
					cbGMode_relaySelect[1] = NULL;
				...
				case devTypeDef_mulitSwThreeBit:{
					...
					lv_obj_del(cbGMode_relaySelect[0]);
					lv_obj_del(cbGMode_relaySelect[1]);
					lv_obj_del(cbGMode_relaySelect[2]);

					cbGMode_relaySelect[0] = NULL;
					cbGMode_relaySelect[1] = NULL;
					cbGMode_relaySelect[2] = NULL;
...
static void lvGuiMenuDelayer_styleMemoryInitialization(void){
...
static void lvGuiMenuDelayer_styleMemoryDeinit(void){
	...
	LV_STYLE_MEM_FREE(styleTabview_gmdyTimeSet_bg);
	LV_STYLE_MEM_FREE(styleTabview_gmdyTimeSet_indic);
	LV_STYLE_MEM_FREE(styleTabview_gmdyTimeSet_btn_bg);
	LV_STYLE_MEM_FREE(styleTabview_gmdyTimeSet_btn_pr);
	LV_STYLE_MEM_FREE(styleTabview_gmdyTimeSet_btn_rel);
	LV_STYLE_MEM_FREE(styleTabview_gmdyTimeSet_btn_tgl_pr);
	LV_STYLE_MEM_FREE(styleTabview_gmdyTimeSet_btn_tgl_rel);
	LV_STYLE_MEM_FREE(styleRoller_gmdyTimeSet_bg);
	LV_STYLE_MEM_FREE(styleRoller_gmdyTimeSet_sel);
	LV_STYLE_MEM_FREE(styleLabel_gmdyTimeSet_rollerIns);
	LV_STYLE_MEM_FREE(styleLabel_gmdyTimeSet_title);
	LV_STYLE_MEM_FREE(styleLabel_gmdyTimeSet_funSave);
	LV_STYLE_MEM_FREE(styleBtn_gmdyTimeSet_funSave);
	...
	LV_STYLE_MEM_FREE(stylePageCb_gModeRelaySel);
}
...
//			 heap_caps_get_minimum_free_size(MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL));
	...
	lvGuiMenuDelayer_styleMemoryInitialization();
...
//	styleMbox_bg->text.font = &lv_font_consola_17;
	styleMbox_bg->text.font = fontLabelGet_bySysLanguage(2);
...
//	styleMbox_btn_rel->text.font = &lv_font_consola_19;
	styleMbox_btn_rel->text.font = fontLabelGet_bySysLanguage(2);
	styleMbox_btn_rel->body.empty = 1;					
...
//	styleTabview_gmdyTimeSet_btn_bg->text.font = &lv_font_consola_16;
	styleTabview_gmdyTimeSet_btn_bg->text.font = fontLabelGet_bySysLanguage(2);
...
//	styleTabview_gmdyTimeSet_btn_pr->text.font = &lv_font_consola_16;
	styleTabview_gmdyTimeSet_btn_pr->text.font = fontLabelGet_bySysLanguage(2);
...
//	styleLabel_gmdyTimeSet_rollerIns->text.font = &lv_font_arial_15;
	styleLabel_gmdyTimeSet_rollerIns->text.font = fontLabelGet_bySysLanguage(0);
...
//	styleLabel_gmdyTimeSet_title->text.font = &lv_font_consola_16;
	styleLabel_gmdyTimeSet_title->text.font = fontLabelGet_bySysLanguage(2);
...
//	styleLabel_gmdyTimeSet_funSave->text.font = &lv_font_consola_19;
	styleLabel_gmdyTimeSet_funSave->text.font = fontLabelGet_bySysLanguage(4);
	...
	lv_style_copy(stylePageCb_gModeRelaySel, &lv_style_plain);
	stylePageCb_gModeRelaySel->body.border.part = LV_BORDER_NONE;
	stylePageCb_gModeRelaySel->body.empty = 1;
	stylePageCb_gModeRelaySel->text.color = LV_COLOR_MAKE(0x40, 0x40, 0x40);
	stylePageCb_gModeRelaySel->text.font = &lv_font_consola_16;

	objText_menuCurrentTitle = lv_label_create(obj_Parent, NULL);
//	lv_label_set_text(objText_menuCurrentTitle, "Delay setting");
	lv_label_set_text(objText_menuCurrentTitle, labelStrGet_bySysLanguage(tls_mDelayer_delaySetting));
	switch(devLanguageCountry_identifyGet()){
...
//	lv_obj_set_top(menuBtnChoIcon_fun_back, true);
	...
	lv_tabview_set_anim_time(objTabv_gmdyOpMenu, 200);
...
//  objTab_greenModeSet = lv_tabview_add_tab(objTabv_gmdyOpMenu, "Greenmode");
	objTab_delayTrigSet = lv_tabview_add_tab(objTabv_gmdyOpMenu, labelStrGet_bySysLanguage(tls_mDelayer_Delay));
	objTab_greenModeSet = lv_tabview_add_tab(objTabv_gmdyOpMenu, labelStrGet_bySysLanguage(tls_mDelayer_GreenMode));
//	(devStatusDispMethod_landscapeIf_get())?
...
//		(lv_obj_set_size(objTab_greenModeSet , 240, 245));
	...
	switch(devLanguageCountry_identifyGet()){
		...
		default:
			...
			(devStatusDispMethod_landscapeIf_get())?
				(coordOffset_cLanguage_x = 240):
				(coordOffset_cLanguage_x = 150);
		break;
	}
	(devStatusDispMethod_landscapeIf_get())?
		(lv_obj_set_pos(objBtn_delayTrigSet_save, coordOffset_cLanguage_x, 30)):
		(lv_obj_set_pos(objBtn_delayTrigSet_save, coordOffset_cLanguage_x, 30));
	objLabel_delayTrigSet_save = lv_label_create(objTab_delayTrigSet, NULL);
//	lv_label_set_text(objLabel_delayTrigSet_save, "Save");
	lv_label_set_text(objLabel_delayTrigSet_save, labelStrGet_bySysLanguage(tls_save));
	lv_obj_set_protect(objLabel_delayTrigSet_save, LV_PROTECT_POS);
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:coordOffset_cLanguage_x = -15;break;
		default:coordOffset_cLanguage_x = 0;break;}
	lv_obj_align(objLabel_delayTrigSet_save, objBtn_delayTrigSet_save, LV_ALIGN_IN_RIGHT_MID, coordOffset_cLanguage_x, 0);
	...
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:
			(devStatusDispMethod_landscapeIf_get())?
				(coordOffset_cLanguage_x = 150):
				(coordOffset_cLanguage_x = 110);
			break;
		default:
			(devStatusDispMethod_landscapeIf_get())?
				(coordOffset_cLanguage_x = -120):
				(coordOffset_cLanguage_x = -70);
			break;}
	lv_obj_align(objLine_delayTrigSet_limit_A, objBtn_delayTrigSet_save, LV_ALIGN_OUT_BOTTOM_MID, coordOffset_cLanguage_x, 5);
...
//	lv_label_set_text(objLabel_delayTrigSet_titleTimeSet, "Time set:");
	lv_label_set_text(objLabel_delayTrigSet_titleTimeSet, labelStrGet_bySysLanguage(tls_mDelayer_timeSet));
	lv_obj_set_protect(objLabel_delayTrigSet_titleTimeSet, LV_PROTECT_POS);
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:lv_obj_align(objLabel_delayTrigSet_titleTimeSet, objLine_delayTrigSet_limit_A, LV_ALIGN_OUT_BOTTOM_RIGHT, -63, 2);break;
		default:lv_obj_align(objLabel_delayTrigSet_titleTimeSet, objLine_delayTrigSet_limit_A, LV_ALIGN_OUT_BOTTOM_LEFT, 3, 2);break;}
...
//	lv_label_set_text(objLabel_delayTrigSet_titleSwValTrig, "Trig set:");
	lv_label_set_text(objLabel_delayTrigSet_titleSwValTrig, labelStrGet_bySysLanguage(tls_mDelayer_trigSet));
	lv_obj_set_protect(objLabel_delayTrigSet_titleSwValTrig, LV_PROTECT_POS);
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:lv_obj_align(objLabel_delayTrigSet_titleSwValTrig, objLine_delayTrigSet_limit_B, LV_ALIGN_OUT_BOTTOM_RIGHT, -53, 2);break;
		default:lv_obj_align(objLabel_delayTrigSet_titleSwValTrig, objLine_delayTrigSet_limit_B, LV_ALIGN_OUT_BOTTOM_LEFT, 3, 2);break;}
	...
	(devStatusDispMethod_landscapeIf_get())?
		(lv_obj_align(objRoller_delayTrigSet_hour, objLine_delayTrigSet_limit_A, LV_ALIGN_OUT_BOTTOM_LEFT, 70, 30)):
		(lv_obj_align(objRoller_delayTrigSet_hour, objLine_delayTrigSet_limit_A, LV_ALIGN_OUT_BOTTOM_LEFT, 35, 30));
...
//	lv_label_set_text(objLabel_delayTrigSet_hour, "hour");
	lv_label_set_text(objLabel_delayTrigSet_hour, labelStrGet_bySysLanguage(tls_hour));
	lv_obj_set_protect(objLabel_delayTrigSet_hour, LV_PROTECT_POS);
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:coordOffset_cLanguage_x = 0;break;
		default:coordOffset_cLanguage_x = 41;break;}
	lv_obj_align(objLabel_delayTrigSet_hour, objRoller_delayTrigSet_hour, LV_ALIGN_IN_RIGHT_MID, coordOffset_cLanguage_x, 2);
...
//	lv_label_set_text(objLabel_delayTrigSet_minute, "minute");
	lv_label_set_text(objLabel_delayTrigSet_minute, labelStrGet_bySysLanguage(tls_minute));
	lv_obj_set_protect(objLabel_delayTrigSet_minute, LV_PROTECT_POS);
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:coordOffset_cLanguage_x = 25;break;
		default:coordOffset_cLanguage_x = 48;break;}
	lv_obj_align(objLabel_delayTrigSet_minute, objRoller_delayTrigSet_minute, LV_ALIGN_IN_RIGHT_MID, coordOffset_cLanguage_x, 0);
...
//		(lv_sw_on(objSw_delayTrigSet_swTrigSet[2]));
	...
	lvGui_businessReuse_reactionObjPageElement_creat(objTab_delayTrigSet, 
													 ...
													 lv_obj_get_y(objLine_delayTrigSet_limit_B) + 50,
	...
	(devStatusDispMethod_landscapeIf_get())?
		(lv_obj_set_pos(objBtn_greenModeSet_save, coordOffset_cLanguage_x, 30)):
		(lv_obj_set_pos(objBtn_greenModeSet_save, coordOffset_cLanguage_x, 30));
	objLabel_greenModeSet_save = lv_label_create(objTab_greenModeSet, NULL);
//	lv_label_set_text(objLabel_greenModeSet_save, "Save");
	lv_label_set_text(objLabel_greenModeSet_save, labelStrGet_bySysLanguage(tls_save));
	lv_obj_set_protect(objLabel_greenModeSet_save, LV_PROTECT_POS);
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:coordOffset_cLanguage_x = -15;break;
		default:coordOffset_cLanguage_x = 0;break;}
	lv_obj_align(objLabel_greenModeSet_save, objBtn_greenModeSet_save, LV_ALIGN_IN_RIGHT_MID, coordOffset_cLanguage_x, 0);
	...
	switch(devLanguageCountry_identifyGet()){
		...
		default:
			(devStatusDispMethod_landscapeIf_get())?
				...
				(coordOffset_cLanguage_x = -70);
			break;}
	lv_obj_align(objLine_greenModeSet_limit_A, objBtn_greenModeSet_save, LV_ALIGN_OUT_BOTTOM_MID, coordOffset_cLanguage_x, 5);
...
//	lv_label_set_text(objLabel_greenModeSet_titleTimeSet, "Time set:");
	lv_label_set_text(objLabel_greenModeSet_titleTimeSet, labelStrGet_bySysLanguage(tls_mDelayer_timeSet));
	lv_obj_set_protect(objLabel_greenModeSet_titleTimeSet, LV_PROTECT_POS);
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:lv_obj_align(objLabel_greenModeSet_titleTimeSet, objLine_greenModeSet_limit_A, LV_ALIGN_OUT_BOTTOM_RIGHT, -63, 2);break;
		default:lv_obj_align(objLabel_greenModeSet_titleTimeSet, objLine_greenModeSet_limit_A, LV_ALIGN_OUT_BOTTOM_LEFT, 3, 2);break;}
...
//	lv_label_set_text(objLabel_greenModeSet_titleSwValTrig, "Enable:");
	lv_label_set_text(objLabel_greenModeSet_titleSwValTrig, labelStrGet_bySysLanguage(tls_mDelayer_enable));
	lv_obj_set_protect(objLabel_greenModeSet_titleSwValTrig, LV_PROTECT_POS);
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:lv_obj_align(objLabel_greenModeSet_titleSwValTrig, objLine_greenModeSet_limit_B, LV_ALIGN_OUT_BOTTOM_RIGHT, -25, 2);break;
		default:lv_obj_align(objLabel_greenModeSet_titleSwValTrig, objLine_greenModeSet_limit_B, LV_ALIGN_OUT_BOTTOM_LEFT, 3, 2);break;}
	...
	(devStatusDispMethod_landscapeIf_get())?
		(lv_obj_align(objRoller_greenModeSet_hour, objLine_greenModeSet_limit_A, LV_ALIGN_OUT_BOTTOM_LEFT, 55, 30)):
...
//	lv_label_set_text(objLabel_greenModeSet_hour, "hour");
	lv_label_set_text(objLabel_greenModeSet_hour, labelStrGet_bySysLanguage(tls_hour));
	lv_obj_set_protect(objLabel_greenModeSet_hour, LV_PROTECT_POS);
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:coordOffset_cLanguage_x = -1;break;
		default:coordOffset_cLanguage_x = 35;break;}
	lv_obj_align(objLabel_greenModeSet_hour, objRoller_greenModeSet_hour, LV_ALIGN_IN_RIGHT_MID, coordOffset_cLanguage_x, 3);
...
//	lv_label_set_text(objLabel_greenModeSet_minute, "min");
	lv_label_set_text(objLabel_greenModeSet_minute, labelStrGet_bySysLanguage(tls_min));
	...
	lv_obj_align(objLabel_greenModeSet_minute, objRoller_greenModeSet_minute, LV_ALIGN_IN_RIGHT_MID, coordOffset_cLanguage_x, 1);
...
//	lv_label_set_text(objLabel_greenModeSet_second, "sec");
	lv_label_set_text(objLabel_greenModeSet_second, labelStrGet_bySysLanguage(tls_sec));
	lv_obj_set_protect(objLabel_greenModeSet_second, LV_PROTECT_POS);
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:coordOffset_cLanguage_x = 23;break;
		default:coordOffset_cLanguage_x = 15;break;}
	lv_obj_align(objLabel_greenModeSet_second, objRoller_greenModeSet_second, LV_ALIGN_IN_RIGHT_MID, coordOffset_cLanguage_x, 1);
	...
	lv_obj_align(objSw_greenModeSet_swRunnigSet, objLine_greenModeSet_limit_B, LV_ALIGN_OUT_BOTTOM_MID, 0, 30);
	if(greenMode_timeCount){
