...
LV_IMG_DECLARE(icon_qrCodePage_reset);
LV_IMG_DECLARE(icon_qrCodePage_scan);
...
static const char **mbox_btnm_textTab = NULL;
...
static lv_obj_t *btn_wifiReset = NULL;
static lv_obj_t *btn_wifiScan = NULL;
...
static void lvGuiMenuWifiConfig_styleMemoryDeinit(void);
...
static void currentGui_elementClear(void){
	...
	if(btn_wifiReset){
		...
		lv_obj_del(btn_wifiReset);
		btn_wifiReset = NULL;
	}

	if(btn_wifiScan){

		lv_obj_del(btn_wifiScan);
		btn_wifiScan = NULL;
	}
...
//	lvGuiMenuWifiConfig_styleMemoryDeinit();
...
static lv_res_t funCb_btnActionClick_qrPage_btnReset(lv_obj_t *btn){

	char *wordsStr = dialogWordsStrGet_bySysLanguage(dialogTips_nwkReset);
	...
	lv_mbox_set_text(mboxBtnKbCall_restart, (const char *)wordsStr);
	os_free(wordsStr);
	...
	if(NULL != btn_wifiReset)
		lv_obj_set_click(btn_wifiReset, false);
	if(NULL != btn_wifiScan)
		lv_obj_set_click(btn_wifiScan, false);
...
static lv_res_t funCb_btnActionClick_menuBtn_wifiCfg_btnByQr(lv_obj_t *btn){
...
static lv_res_t funCb_btnActionLongPress_menuBtn_wifiCfg_btnByQr(lv_obj_t *btn){

	printf("btn Qr long pressed.\n");
	...
	do{
		...
		if(!strcmp(mbox_btnm_textTab[loop], txt)){
			...
			switch(loop){
				...
				case 1:
				default:
					if(VALDEF_BOOL_TRUE == QrCodeCreat_flg)
						wifiConfigOpreat_comfigFailCounter = 2;
					if(NULL != btn_wifiCfg_methodQr)
...
//	btn_wifiCfg_methodKb = NULL;
	...
	if(meshNetwork_connectReserve_IF_get()){
		...
		(devStatusDispMethod_landscapeIf_get())?
			(lv_obj_set_width(mboxBtnKbCall_restart, 300)):
			(lv_obj_set_width(mboxBtnKbCall_restart, 230));
//		sprintf(wordsStr, "%s", dialogWordsStrGet_bySysLanguage(dialogTips_nwkReset));
		lv_mbox_set_text(mboxBtnKbCall_restart, (const char *)wordsStr);
		os_free(wordsStr);
		lv_mbox_add_btns(mboxBtnKbCall_restart, mbox_btnm_textTab, NULL);
		lv_mbox_set_style(mboxBtnKbCall_restart, LV_MBOX_STYLE_BG, styleMbox_bg);
...
static lv_res_t funCb_btnActionPress_menuBtn_wifiCfg_btnByKbOnly(lv_obj_t *btn){
...
static lv_res_t funCb_kbActionClick_wifiSelCfgOk(lv_obj_t *kb){
	...
	if(NULL != btn_wifiCfg_methodQr)
		lv_obj_set_click(btn_wifiCfg_methodQr, true);
	if(NULL != btn_wifiCfg_methodKb)
		lv_obj_set_click(btn_wifiCfg_methodKb, true);

	if(VALDEF_BOOL_TRUE == QrCodeCreat_flg)
		wifiConfigOpreat_comfigFailCounter = 0;
...
static lv_res_t funCb_kbActionClick_wifiSelCfgHidden(lv_obj_t *kb){
	...
	if(VALDEF_BOOL_TRUE == QrCodeCreat_flg)
		wifiConfigOpreat_comfigFailCounter = 1;
...
//	styleLabelUsrFun_wifiConfig_psdTips->text.font = &lv_font_consola_19;
	styleLabelUsrFun_wifiConfig_psdTips->text.font = fontLabelGet_bySysLanguage(4);
    ...
    styleKb_usrFun_wifiCfg_btnPre->body.main_color = LV_COLOR_YELLOW;
    styleKb_usrFun_wifiCfg_btnPre->body.grad_color = LV_COLOR_AQUA;
...
static void lvGui_businessMenu_wifiConfig_unitKeyboard_detail(lv_obj_t * obj_Parent, char *ssidIpt){
	...
	int16_t coordOffset_cLanguage_x = 150;
	char strTemp[32] = {0};
	...
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:
			(devStatusDispMethod_landscapeIf_get())?
				(lv_obj_set_pos(labelUsrFun_wifiConfig_psdTips, 205, 5)):
				(lv_obj_set_pos(labelUsrFun_wifiConfig_psdTips, 160, 0));
			break;
		default:
			(devStatusDispMethod_landscapeIf_get())?
	...
	(devStatusDispMethod_landscapeIf_get())?
		(sprintf(strTemp, "      %s", labelStrGet_bySysLanguage(tls_mWifiCfg_wifiSSID))):
		(sprintf(strTemp, "%s", labelStrGet_bySysLanguage(tls_mWifiCfg_wifiSSID)));
	lv_label_set_text(labelUsrFun_wifiConfig_psdTips, strTemp);	
	labelUsrFun_wifiConfig_ssidTips = lv_label_create(obj_Parent, labelUsrFun_wifiConfig_psdTips);
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:{
			(devStatusDispMethod_landscapeIf_get())?
				(lv_obj_align(labelUsrFun_wifiConfig_ssidTips, labelUsrFun_wifiConfig_psdTips, LV_ALIGN_OUT_BOTTOM_LEFT, 9, 28)):

				(lv_obj_align(labelUsrFun_wifiConfig_ssidTips, labelUsrFun_wifiConfig_psdTips, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 40));
		}break;
		default:
			(devStatusDispMethod_landscapeIf_get())?
				(lv_obj_align(labelUsrFun_wifiConfig_ssidTips, labelUsrFun_wifiConfig_psdTips, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 26)):

				(lv_obj_align(labelUsrFun_wifiConfig_ssidTips, labelUsrFun_wifiConfig_psdTips, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 44));
			break;}
//	lv_label_set_text(labelUsrFun_wifiConfig_ssidTips, "Password:");
	lv_label_set_text(labelUsrFun_wifiConfig_ssidTips, labelStrGet_bySysLanguage(tls_mWifiCfg_wifiPassword));
	...
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:
			(devStatusDispMethod_landscapeIf_get())?
				(lv_obj_align(taUsrFun_wifiConfig_ssid, labelUsrFun_wifiConfig_psdTips, LV_ALIGN_OUT_LEFT_BOTTOM,    0, -8)):
				(lv_obj_align(taUsrFun_wifiConfig_ssid, labelUsrFun_wifiConfig_psdTips, LV_ALIGN_OUT_BOTTOM_LEFT, -150, -3));
			break;
		default:
			(devStatusDispMethod_landscapeIf_get())?
	...
	(devStatusDispMethod_landscapeIf_get())?
		(lv_obj_align(taUsrFun_wifiConfig_psd, taUsrFun_wifiConfig_ssid, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 2)):
		(lv_obj_align(taUsrFun_wifiConfig_psd, taUsrFun_wifiConfig_ssid, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 18));
...
//	printf("Qr code creat res:%d.\n", EncodeData("hellow, Lanbon!"));
	...
	for(uint8_t loopa = 0; loopa < MAX_MODULESIZE; loopa ++){
		...
		for(uint8_t loopb = 0; loopb < MAX_MODULESIZE; loopb ++){
			...
			(m_byModuleData[loopa][loopb])?
				(externSocket_ex_disp_fill(posCoord_x + loopa * qrCode_cellSize, posCoord_y + loopb * qrCode_cellSize, posCoord_x + qrCode_cellSize + loopa * qrCode_cellSize, posCoord_y + qrCode_cellSize + loopb * qrCode_cellSize, (meshNetwork_connectReserve_IF_get())?(LV_COLOR_MAKE(8, 164, 8)):(LV_COLOR_BLACK))):
	...
	if(NULL != btn_wifiReset)
		lv_obj_del(btn_wifiReset);
	btn_wifiReset = lv_imgbtn_create(lv_scr_act(), NULL);
	lv_imgbtn_set_src(btn_wifiReset, LV_BTN_STATE_REL, &icon_qrCodePage_reset);
	lv_imgbtn_set_src(btn_wifiReset, LV_BTN_STATE_PR, &icon_qrCodePage_reset);
	devStatusDispMethod_landscapeIf_get()?
		(lv_obj_set_pos(btn_wifiReset, 240, 180)):
		(lv_obj_set_pos(btn_wifiReset, 80, 290));
	lv_btn_set_action(btn_wifiReset, LV_BTN_ACTION_CLICK, funCb_btnActionClick_qrPage_btnReset);
	lv_btn_set_action(btn_wifiReset, LV_BTN_ACTION_PR, funCb_btnAction_null);
	lv_obj_set_click(btn_wifiReset, true);

	if(NULL != btn_wifiScan)
		lv_obj_del(btn_wifiScan);
	btn_wifiScan = lv_imgbtn_create(lv_scr_act(), NULL);
	lv_imgbtn_set_src(btn_wifiScan, LV_BTN_STATE_REL, &icon_qrCodePage_scan);
	lv_imgbtn_set_src(btn_wifiScan, LV_BTN_STATE_PR, &icon_qrCodePage_scan);
	devStatusDispMethod_landscapeIf_get()?
		(lv_obj_set_pos(btn_wifiScan, 260, 210)):
		(lv_obj_set_pos(btn_wifiScan, 190, 295));
	lv_btn_set_action(btn_wifiScan, LV_BTN_ACTION_CLICK, funCb_btnActionPress_menuBtn_wifiCfg_btnByKbOnly);
	lv_btn_set_action(btn_wifiScan, LV_BTN_ACTION_PR, funCb_btnAction_null);
	lv_obj_set_hidden(btn_wifiScan, meshNetwork_connectReserve_IF_get());
	lv_obj_set_click(btn_wifiScan, true);
}
...
//	lvGui_usrAppBussinessRunning_block(0, "   wifi\n   scanning...", 8); //wifi扫描，UI阻塞
	lvGui_usrAppBussinessRunning_block(0, labelStrGet_bySysLanguage(tls_wifiScanning), 8); //wifi扫描，UI阻塞
...
void lvGui_businessMenu_wifiConfig_wifiStatusReleasing(void){
	...
	if(meshNetwork_connectReserve_IF_get()){
		...
		if(flgGet_gotRouterOrMeshConnect()){
			...
			sprintf(strTemp_wifiCfg_currentWifiStatusInfo, "%s\n  %sSSID: #B5E61D %s#\n  %sPSD: #B5E61D %s#",
															labelStrGet_bySysLanguage(tls_mWifiCfg_wifiStatus),
															textTabAlign,
		...
		{
			sprintf(strTemp_wifiCfg_currentWifiStatusInfo, "%s\n\n	#B9B9FF %sRECONNECTING...#", labelStrGet_bySysLanguage(tls_mWifiCfg_wifiStatus), textTabAlign);
		}
	}
	else
	{
		sprintf(strTemp_wifiCfg_currentWifiStatusInfo, "%s\n\n	#B9B9FF    %sNO CONFIG#", labelStrGet_bySysLanguage(tls_mWifiCfg_wifiStatus), textTabAlign);
	}
}
...
void lvGui_businessMenu_wifiConfig_methodSelect(lv_obj_t * obj_Parent){
	...
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:lv_label_set_align(labelRef_wifiCfg_methodSel_wifiInfo, LV_LABEL_ALIGN_RIGHT); break;
		default:lv_label_set_align(labelRef_wifiCfg_methodSel_wifiInfo, LV_LABEL_ALIGN_LEFT);break;}
	...
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:{
			...
			stt_routerCfgInfo routerCfgInfo_temp = {0};
			currentRouterCfgInfo_paramGet(&routerCfgInfo_temp);
			(strlen((char *)routerCfgInfo_temp.routerInfo_ssid) > strlen((char *)routerCfgInfo_temp.routerInfo_psd))?
				(posXoffset = strlen((char *)routerCfgInfo_temp.routerInfo_ssid) * 6):
				(posXoffset = strlen((char *)routerCfgInfo_temp.routerInfo_psd) * 6);
			if(!meshNetwork_connectReserve_IF_get())posXoffset = 0;
			else if(!flgGet_gotRouterOrMeshConnect())posXoffset = 20;
			(devStatusDispMethod_landscapeIf_get())?
				(lv_obj_set_pos(labelRef_wifiCfg_methodSel_wifiInfo, 180 - posXoffset, 80)):
				(lv_obj_set_pos(labelRef_wifiCfg_methodSel_wifiInfo, 120 - posXoffset, 100));
		}break;
		default:
			(devStatusDispMethod_landscapeIf_get())?
...
//	lv_label_set_text(labelRef_wifiCfg_methodSel, "Method select:");
	lv_label_set_text(labelRef_wifiCfg_methodSel, labelStrGet_bySysLanguage(tls_mWifiCfg_methodSelect));
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:lv_label_set_align(labelRef_wifiCfg_methodSel, LV_LABEL_ALIGN_RIGHT); break;
		default:lv_label_set_align(labelRef_wifiCfg_methodSel, LV_LABEL_ALIGN_LEFT);break;}
	lv_obj_set_protect(labelRef_wifiCfg_methodSel, LV_PROTECT_POS);
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:lv_obj_align(labelRef_wifiCfg_methodSel, labelRef_wifiCfg_methodSel_wifiInfo, LV_ALIGN_OUT_BOTTOM_RIGHT, -60, 15);break;
		default:lv_obj_align(labelRef_wifiCfg_methodSel, labelRef_wifiCfg_methodSel_wifiInfo, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 15);break;}
	...
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:
			(devStatusDispMethod_landscapeIf_get())?
				(lv_obj_align(btn_wifiCfg_methodQr, labelRef_wifiCfg_methodSel, LV_ALIGN_OUT_BOTTOM_RIGHT, -105, 10)):
				(lv_obj_align(btn_wifiCfg_methodQr, labelRef_wifiCfg_methodSel, LV_ALIGN_OUT_BOTTOM_RIGHT, -25, 25));
			break;
		default:
			(devStatusDispMethod_landscapeIf_get())?
			...
			break;}
	lv_btn_set_style(btn_wifiCfg_methodQr, LV_BTNM_STYLE_BG, styleKb_usrFun_wifiCfg_btnBg);
	lv_btn_set_style(btn_wifiCfg_methodQr, LV_BTNM_STYLE_BTN_REL, styleKb_usrFun_wifiCfg_btnRel);
	lv_btn_set_style(btn_wifiCfg_methodQr, LV_BTNM_STYLE_BTN_PR, styleKb_usrFun_wifiCfg_btnPre);
	lv_btn_set_style(btn_wifiCfg_methodQr, LV_BTNM_STYLE_BTN_TGL_REL, styleKb_usrFun_wifiCfg_btnTglRel);
	lv_btn_set_style(btn_wifiCfg_methodQr, LV_BTNM_STYLE_BTN_TGL_PR, styleKb_usrFun_wifiCfg_btnTglPre);
	lv_btn_set_style(btn_wifiCfg_methodQr, LV_BTNM_STYLE_BTN_INA, styleKb_usrFun_wifiCfg_btnIna);
	lv_btn_set_action(btn_wifiCfg_methodQr, LV_BTN_ACTION_CLICK, funCb_btnActionClick_menuBtn_wifiCfg_btnByQr);
//	lv_btn_set_action(btn_wifiCfg_methodQr, LV_BTN_ACTION_LONG_PR, funCb_btnActionLongPress_menuBtn_wifiCfg_btnByQr);
...
//	lv_label_set_text(labelRef_wifiCfg_methodQr_btRef, "#99D9EA Qr code#");
	sprintf(strTemp, "#99D9EA %s#", labelStrGet_bySysLanguage(tls_mWifiCfg_QrCode));
	lv_label_set_text(labelRef_wifiCfg_methodQr_btRef, (const char *)strTemp);
	...
	lv_obj_set_size(btn_wifiCfg_methodKb, 75, 35);
	lv_btn_set_style(btn_wifiCfg_methodKb, LV_BTNM_STYLE_BG, &lv_style_transp);
	lv_btn_set_style(btn_wifiCfg_methodKb, LV_BTNM_STYLE_BTN_REL, &lv_style_transp);
	lv_btn_set_style(btn_wifiCfg_methodKb, LV_BTNM_STYLE_BTN_PR, &lv_style_transp);
	lv_btn_set_style(btn_wifiCfg_methodKb, LV_BTNM_STYLE_BTN_TGL_REL, &lv_style_transp);
	lv_btn_set_style(btn_wifiCfg_methodKb, LV_BTNM_STYLE_BTN_TGL_PR, &lv_style_transp);
	lv_btn_set_style(btn_wifiCfg_methodKb, LV_BTNM_STYLE_BTN_INA, &lv_style_transp);
	switch(devLanguageCountry_identifyGet()){
		case languageCyId_Arabic:
			(devStatusDispMethod_landscapeIf_get())?
				(lv_obj_align(btn_wifiCfg_methodKb, btn_wifiCfg_methodQr, LV_ALIGN_OUT_BOTTOM_RIGHT, 100, -32)):
				(lv_obj_align(btn_wifiCfg_methodKb, btn_wifiCfg_methodQr, LV_ALIGN_OUT_BOTTOM_RIGHT, 20, 15));
			break;
		default:
			(devStatusDispMethod_landscapeIf_get())?
				(lv_obj_align(btn_wifiCfg_methodKb, btn_wifiCfg_methodQr, LV_ALIGN_OUT_BOTTOM_RIGHT, 100, -32)):
	...
	if(meshNetwork_connectReserve_IF_get()){
		...
		lv_obj_set_size(labelRef_wifiCfg_methodLab_underline, 90, 1);
		lv_obj_set_size(labelRef_wifiCfg_methodKb_btRef, 90, 1);
//		lv_label_set_text(labelRef_wifiCfg_methodKb_btRef, "#808080 Reset#");
		sprintf(strTemp, "#808080 %s#", labelStrGet_bySysLanguage(tls_mWifiCfg_reset));
		lv_label_set_text(labelRef_wifiCfg_methodKb_btRef, (const char *)strTemp);
...
//	lv_label_set_text(text_Title, "Wifi config");
	lv_label_set_text(text_Title, labelStrGet_bySysLanguage(tls_mWifiCfg_wifiCfg));
	switch(devLanguageCountry_identifyGet()){
   ...
   (L8_DEVICE_TYPE_PANEL_DEF == DEV_TYPES_PANEL_DEF_UART_MOUDLE)
	...
	switch(err){
		...
		case WIFI_REASON_HANDSHAKE_TIMEOUT:{        /**< disconnected reason code 204 */
			sprintf(strTips_temp, "#FF007F Password#\n#FF007F incorrect #");
		}break;

		case WIFI_REASON_NO_AP_FOUND:{
			sprintf(strTips_temp,"#FF007F Weak router #\n#FF007F signal #");
		}break;

		case WIFI_REASON_CONNECTION_FAIL:{
			sprintf(strTips_temp,"#FF007F Connect fail! #");
		}break;
...
void lvGui_wifiConfig_bussiness_configComplete_tipsDetect(void){
	...
	const uint8_t qrCodeTrigDelay_period = 23;
...
static void lvGuiMenuWifiConfig_styleMemoryInitialization(void){
	...
	styleText_menuLevel_B_infoMac 		= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleText_menuLevel_B_infoTips 		= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	stylePage_wifiConfigComplete 		= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleImage_wifiConfigComplete 		= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleLabelRef_wifiConfigComplete 	= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleLabelRef_wifiCfg_methodSel_info = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleLabelRef_wifiCfg_methodSel_btnRef = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleLabelRef_wifiCfg_methodSel_btnMore = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	...
	styleEmpty 						 = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	stylePageUsrFun_wifiApListShowUp = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleBtnUsrFun_wifiApListShowUp  = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleImageUsrFun_wifiSigRef 	 = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleLabelUsrFun_wifiApSsid 	 = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	
	styleKb_usrFun_wifiCfg_btnBg 	= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleKb_usrFun_wifiCfg_btnRel 	= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleKb_usrFun_wifiCfg_btnPre 	= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleKb_usrFun_wifiCfg_btnTglRel = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleKb_usrFun_wifiCfg_btnTglPre = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleKb_usrFun_wifiCfg_btnIna 	= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	stylepageKbArea_KbWifiConfig 	= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	stylepageUsrFun_KbWifiConfig 	= (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleLabelUsrFun_wifiConfig_psdTips = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
	styleLabelUsrFun_wifiConfig_kbMoveTips = (lv_style_t *)LV_MEM_CUSTOM_ALLOC(sizeof(lv_style_t));
}

static void lvGuiMenuWifiConfig_styleMemoryDeinit(void){

	LV_STYLE_MEM_FREE(styleText_menuLevel_A);
	LV_STYLE_MEM_FREE(styleText_menuLevel_B_infoMac);
	LV_STYLE_MEM_FREE(styleText_menuLevel_B_infoTips);
	LV_STYLE_MEM_FREE(stylePage_wifiConfigComplete);
	LV_STYLE_MEM_FREE(styleImage_wifiConfigComplete);
	LV_STYLE_MEM_FREE(styleLabelRef_wifiConfigComplete);
	LV_STYLE_MEM_FREE(styleLabelRef_wifiCfg_methodSel_info);
	LV_STYLE_MEM_FREE(styleLabelRef_wifiCfg_methodSel_btnRef);
	LV_STYLE_MEM_FREE(styleLabelRef_wifiCfg_methodSel_btnMore);
	...
	LV_STYLE_MEM_FREE(styleEmpty);
	LV_STYLE_MEM_FREE(stylePageUsrFun_wifiApListShowUp);
	LV_STYLE_MEM_FREE(styleBtnUsrFun_wifiApListShowUp);
	LV_STYLE_MEM_FREE(styleImageUsrFun_wifiSigRef);
	LV_STYLE_MEM_FREE(styleLabelUsrFun_wifiApSsid);
	LV_STYLE_MEM_FREE(styleKb_usrFun_wifiCfg_btnBg);
	LV_STYLE_MEM_FREE(styleKb_usrFun_wifiCfg_btnRel);
	LV_STYLE_MEM_FREE(styleKb_usrFun_wifiCfg_btnPre);
	LV_STYLE_MEM_FREE(styleKb_usrFun_wifiCfg_btnTglRel);
	LV_STYLE_MEM_FREE(styleKb_usrFun_wifiCfg_btnTglPre);
	LV_STYLE_MEM_FREE(styleKb_usrFun_wifiCfg_btnIna);
	LV_STYLE_MEM_FREE(stylepageKbArea_KbWifiConfig);
	LV_STYLE_MEM_FREE(stylepageUsrFun_KbWifiConfig);
	LV_STYLE_MEM_FREE(styleLabelUsrFun_wifiConfig_psdTips);
	LV_STYLE_MEM_FREE(styleLabelUsrFun_wifiConfig_kbMoveTips);
}
...
//	styleLabelRef_wifiCfg_methodSel_info->text.font = &lv_font_consola_19;
	styleLabelRef_wifiCfg_methodSel_info->text.font = fontLabelGet_bySysLanguage(4);
...
//	styleLabelRef_wifiCfg_methodSel_btnRef->text.font = &lv_font_consola_19;
	styleLabelRef_wifiCfg_methodSel_btnRef->text.font = fontLabelGet_bySysLanguage(4);
...
//	styleLabelRef_wifiCfg_methodSel_btnMore->text.font = &lv_font_consola_16;
	styleLabelRef_wifiCfg_methodSel_btnMore->text.font = fontLabelGet_bySysLanguage(2);
