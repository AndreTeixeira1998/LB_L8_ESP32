...
static void CalibrationCalculate(const lv_point_t *cross, const lv_point_t *points)
{
     ...
     */
        ...
        // Work on x values
        c0 = c1 = c2 = L8_DEV_SCREEN_SIZE_HOR - 1;
        c0 -= cross[0].x;
        c1 -= cross[1].x;
        c2 -= cross[2].x;
    #elif defined(CONFIG_LVGL_DISP_ROTATE_270)
        // Work on x values
        c0 = c1 = c2 = L8_DEV_SCREEN_SIZE_VER - 1;
        c0 -= cross[0].y;
        c1 -= cross[1].y;
        c2 -= cross[2].y;
    #endif
 ...
 *   GLOBAL FUNCTIONS
 **********************/
void usrApp_lvgl_calibration_fakeDataSave(lv_indev_drv_t indev_drv){

	lv_indev_data_t data;

	GMouseCalibration fakeData = {
	...
	touch_save_calibration(&fakeData, sizeof(GMouseCalibration));
	vTaskDelay(CALIBRATION_POLL_PERIOD / portTICK_PERIOD_MS);
...
{
	// constGMouseCalibration fakeData = {
    // .ax =  -1.0F,
    // .bx =  0.0F,
    // .cx = 240.0F,
    // .ay = 0.0F,
    ...
    if (calibrated) {
        ...
        // x = (lv_coord_t)(fakeData.ax * data->x + fakeData.bx * data->y + fakeData.cx);
        // y = (lv_coord_t)(fakeData.ay * data->x + fakeData.by * data->y + fakeData.cy);

        x = (lv_coord_t)(caldata.ax * data->x + caldata.bx * data->y + caldata.cx);
        y = (lv_coord_t)(caldata.ay * data->y + caldata.by * data->y + caldata.cy);
...
void lvgl_calibrate_customInit(void){

	const GMouseCalibration fakeData = {
    ...
    memcpy(&caldata, &fakeData, sizeof(GMouseCalibration));
    calibrated = true;
}
...
bool lvgl_calibrate_mouse(lv_indev_drv_t indev_drv, bool recalibrate)
{
    ...
    if (!recalibrate && touch_load_calibration(&caldata)) {
		...
		printf("td: ax(%.2f),ay(%.2f),bx(%.2f),by(%.2f),cx(%.2f),cy(%.2f).\n",
				caldata.ax,caldata.ay,
				caldata.bx,caldata.by,
				caldata.cx,caldata.cy);
    ...
    w = L8_DEV_SCREEN_SIZE_HOR;
    h = L8_DEV_SCREEN_SIZE_VER;
    ...
    while (calibrate_error) {
        ...
        lv_obj_set_size(cont1, L8_DEV_SCREEN_SIZE_HOR, L8_DEV_SCREEN_SIZE_VER);
